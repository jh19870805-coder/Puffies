...
internal static class GameLocalizationCatalog
{
    ...
    public static GameLocalization.LocalizedEntry[] CreateEntries()
    {
        return new[]
        {
            ...
            E("game.test_auto_puzzle", "自动拼图", "Auto Place", "Автосборка", "Colocar auto", "Colocar auto", "Colocar auto", "Colocar auto", "Auto-Platzieren", "자동 맞추기", "Placement auto", "自動配置", "Otomatik yerleştir", "自動拼圖", "Auto-układanie", "Posiziona auto", "Автоскладання", "Ghép tự động", "วางอัตโนมัติ"),
            E("game.wishlist_prompt.content", "喜欢这款游戏吗？\n今天就将其加入愿望清单吧，这样一旦推出，你就会立即收到通知哦！", "Like this game?\nAdd it to your wishlist today and you will be notified as soon as it launches!", "Нравится игра?\nДобавьте её в список желаемого, чтобы получить уведомление сразу после выхода!", "¿Te gusta este juego?\nAñádelo hoy a tu lista de deseados y recibirás un aviso en cuanto se publique.", "¿Te gusta este juego?\nAgrégalo hoy a tu lista de deseados y recibirás un aviso en cuanto se lance.", "Gostou deste jogo?\nAdicione-o hoje à sua lista de desejos para receber um aviso assim que for lançado!", "Gostas deste jogo?\nAdiciona-o hoje à tua lista de desejos para receberes um aviso assim que for lançado!", "Gefällt dir dieses Spiel?\nSetze es auf deine Wunschliste und du wirst zum Release sofort benachrichtigt!", "이 게임이 마음에 드시나요?\n지금 위시리스트에 추가하면 출시 즉시 알림을 받을 수 있어요!", "Vous aimez ce jeu ?\nAjoutez-le à votre liste de souhaits pour être averti dès sa sortie !", "このゲームが気に入りましたか？\n今すぐウィッシュリストに追加すると、リリース時に通知が届きます！", "Bu oyunu beğendiniz mi?\nBugün istek listenize ekleyin; çıktığında hemen bildirim alın!", "喜歡這款遊戲嗎？\n現在就加入願望清單，推出時便會立即收到通知！", "Podoba Ci się ta gra?\nDodaj ją dziś do listy życzeń, aby otrzymać powiadomienie zaraz po premierze!", "Ti piace questo gioco?\nAggiungilo oggi alla lista desideri per ricevere una notifica appena uscirà!", "Подобається ця гра?\nДодайте її до списку бажаного, щоб отримати сповіщення одразу після виходу!", "Bạn thích trò chơi này chứ?\nHãy thêm vào danh sách ước ngay hôm nay để nhận thông báo khi trò chơi ra mắt!", "ชอบเกมนี้ไหม?\nเพิ่มลงในสิ่งที่อยากได้วันนี้ แล้วรับการแจ้งเตือนทันทีเมื่อเกมวางจำหน่าย!"),
            E("game.wishlist_prompt.add", "添加愿望单", "Add to Wishlist", "Добавить в желаемое", "Añadir a deseados", "Agregar a deseados", "Adicionar à lista de desejos", "Adicionar à lista de desejos", "Zur Wunschliste", "위시리스트에 추가", "Ajouter aux souhaits", "ウィッシュリストに追加", "İstek listesine ekle", "加入願望清單", "Dodaj do listy życzeń", "Aggiungi alla lista desideri", "Додати до бажаного", "Thêm vào danh sách ước", "เพิ่มในสิ่งที่อยากได้"),
            E("game.tutorial.place", "从托盘中选出匹配的贴纸，贴在板子的正确位置上。", "Place the matching sticker on the board.", "Выберите подходящую наклейку с подноса и поместите её на нужное место.", "Elige la pegatina correcta de la bandeja y colócala en su lugar del tablero.", "Elige el sticker correcto de la bandeja y colócalo en su lugar del tablero.", "Escolha o adesivo certo na bandeja e coloque-o no local correto do tabuleiro.", "Escolhe o autocolante certo do tabuleiro e coloca-o no local correto.", "Wähle den passenden Sticker aus der Ablage und setze ihn an die richtige Stelle.", "트레이에서 알맞은 스티커를 골라 보드의 올바른 위치에 놓으세요.", "Choisissez l'autocollant correspondant et placez-le au bon endroit sur le plateau.", "トレイから合うステッカーを選び、ボードの正しい位置に置きましょう。", "Tepsiden eşleşen çıkartmayı seçip tahtadaki doğru yere yerleştirin.", "從托盤選出相符貼紙，放到板子的正確位置。", "Wybierz pasującą naklejkę z tacki i umieść ją we właściwym miejscu.", "Scegli l'adesivo giusto dal vassoio e mettilo nel punto corretto.", "Виберіть відповідну наліпку з лотка й помістіть її на правильне місце.", "Chọn nhãn dán phù hợp từ khay và đặt vào đúng vị trí trên bảng.", "เลือกสติกเกอร์ที่ตรงกันจากถาด แล้ววางในตำแหน่งที่ถูกต้องบนกระดาน"),
            ...
            E("task.all_packs_collected", "当前阶段的卡包已收集完毕！", "All card packs available at this stage have been collected!", "Все наборы текущего этапа собраны!", "¡Has conseguido todos los paquetes disponibles en esta etapa!", "¡Conseguiste todos los paquetes disponibles en esta etapa!", "Todos os pacotes disponíveis nesta etapa foram obtidos!", "Todos os pacotes disponíveis nesta fase foram obtidos!", "Alle in dieser Phase verfügbaren Puzzle-Packs wurden gesammelt!", "현재 단계의 모든 퍼즐 팩을 획득했습니다!", "Tous les packs disponibles à cette étape ont été obtenus !", "現段階のパックをすべて獲得しました！", "Bu aşamadaki tüm bulmaca paketleri alındı!", "當前階段的拼圖包已全部收集完畢！", "Zdobyto wszystkie paczki dostępne na tym etapie!", "Hai ottenuto tutti i pacchetti disponibili in questa fase!", "Усі набори поточного етапу отримано!", "Đã nhận đủ tất cả gói xếp hình trong giai đoạn hiện tại!", "ได้รับแพ็กพัซเซิลทั้งหมดในช่วงนี้ครบแล้ว!"),
