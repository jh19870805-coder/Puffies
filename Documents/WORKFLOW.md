...
- 用户要求“同步工作进度”时，先读取 `CURRENT_TASK.md` 顶部是否存在状态为“待执行”的一次性跨设备任务。
- 一次性跨设备任务优先于普通的下一步行动；按任务内步骤完成验证后，立即将状态改为“已完成”，记录执行设备、验证结果和日期。
- 已标记“已完成”的一次性任务不得重复执行。任务未完成前，不得只汇报普通开发进度而跳过该任务。
