...
- 点击结算 `BtnFinish` 后，顶部 `TaskBg2/TaskItem` 先按入场的反向节奏收回屏幕上方，底部 `ImgBagBg/BtnFinish/BtnCamera` 同时收回屏幕下方；顶部 `0.52s`、底部 `0.42s`，不改变透明度、缩放或旋转。已经显示的真实奖励图标在退场前脱离 `ImgBagBg` 并提升到 `RewardPanel` 顶层，保持原屏幕位置，不能随底板退出。退场全部完成后，`CardPackRewardFlyTransition` 才负责首页切换和奖励卡入列，不属于开包特效。GameScene 最后一帧通过全屏 RenderTexture 覆盖层跨场景保留；MainScene 列表完成创建和排序后，先缓存真实槽位并把全部列表卡包移到屏幕下方外侧，再用 `0.30s` 直接淡出结算覆盖图显示首页，禁止使用纯黑中间层。已经分配真实 PackId 的奖励分别复制结算页各自默认 `ImgBag` 图标，从原槽位以单张 `0.72s`、多张 `0.12s` 错峰的弧线移动并同步缩放到对应列表尺寸，途中不得提前加载真实封面。落位后预留 `0.20s` 给后续闪光特效，当前不播放替代特效；预留结束再显示该槽真实卡包完整视觉。全部奖励揭晓后，其余卡包按单卡 `0.44s`、错峰 `0.055s` 从屏幕下方依次滑入。无真实奖励时仍执行跨场景淡出和整列入场；系列后层奖励映射到系列实际列表槽。动画结束或取消时必须释放截图 RenderTexture，并恢复 GridLayout、ScrollRect、列表位置和输入。
