...
- 音频资源统一平铺在 `Assets/Audios`：背景音乐使用 `BGM_` 前缀，短音效使用 `SFX_` 前缀，名称只使用 ASCII、PascalCase 功能名和必要的两位数字序号。事件型 SFX 源文件不得保留可感知的前导静音；播放时机正确但听感滞后时，应先检查音频波形，再决定是否调整代码。当前 19 个 SFX 已按各自实际波形裁掉前导静音，复测起音为 `0~5 ms`；后续替换素材时需要重新检查。项目 DSP Buffer 固定为 `512`，不得在未验证设备性能和爆音风险时继续降低。`AudioManager` 是 `DontDestroyOnLoad` 常驻单例，业务层直接向 `PlayMusic/PlaySfx` 传 MP3 文件名；`Assets/Resources/AudioCatalog.asset` 持有全部 AudioClip 引用，Editor 会在音频变动、工程加载及构建前自动扫描同步。LoadingScene 异步加载 Catalog 并预热首页 BGM 与全部 SFX，最多等待 `10s` 后降级继续；SFX 使用 `DecompressOnLoad + preloadAudioData + loadInBackground`，BGM 使用 `Streaming + loadInBackground`。当前 6 首 BGM 的压缩 MP3 合计约 `10.51 MiB`。MainScene 固定播放 `BGM_MainMenu.mp3`；GameScene 在 `BGM_Gameplay_01~05.mp3` 中首次随机并将稳定文件名保存到当前 SaveSlot 的 SQLite `GameAudioPreferences` 集合，非系列按 PackId、系列按系列链首 PackId 复用，后续进入不得重新随机；选中卡包后即后台准备该曲目，GameScene 激活时复用已确定文件名。BGM 与 SFX 使用独立 AudioSource 音量，`AudioListener.volume` 固定为 `1`，避免二次乘算。
