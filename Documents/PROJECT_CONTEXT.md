...
- 运行时 TMP 文本统一采用“保留明确换行、禁止自动新增换行、超宽时缩小字号”的适配规则。已有 Auto Size 的文本保留编辑器字号范围；未启用 Auto Size 的文本以当前编辑器字号为上限、`10` 为下限。动态创建或动态更新的 TMP 文本同样应用该规则，但不得借此改动字体、材质、颜色、对齐、RectTransform 或动画。
