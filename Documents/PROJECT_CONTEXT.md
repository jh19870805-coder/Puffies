...
- `PanelBagSelect` 每次打开时按 `CardPackRecord.IsPlayed` 刷新操作：`Unlocked` 卡包的确认按钮显示 `Play` 且隐藏 `BtnCamera`；`InProgress` 和 `Completed` 卡包的确认按钮显示 `重玩` 且显示 `BtnCamera`。重玩继续复用 `BtnPlay` 的现有开包流程。
