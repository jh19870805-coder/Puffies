...
- MainScene 对 `Series` 链执行系列槽位折叠：同系列全部已解锁成员只占一个网格位置，当前最高已解锁 Vol 显示在 `PackCover` 前层，上一已解锁 Vol 使用上一 Vol 真实 PackId 的封面显示在同中心 `PackCover2` 后层，后层等比放大至 `1.1` 倍并按系列稳定旋转 `+10°/-10°`；Vol2 起显示对应 `PackVolN.png`。前后封面各自依据所属卡包进度应用完整、彩色撕开或完成灰色撕开状态，组合槽点击目标暂为最高已解锁 Vol。分页网格按完整六列宽度计算固定左右边距并从 `UpperLeft` 排列，因此满页居中且末页从相同第一列开始。
