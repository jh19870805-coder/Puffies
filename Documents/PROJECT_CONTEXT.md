...
- 默认 Demo 的 `CardBag001~018` 全部进入 `Unlocked`、`InProgress` 或 `Completed` 后，任务系统对业务返回无活动任务，不再向玩家提供新任务；原 JSON 任务状态保留，定义 `PUFFIES_STEAM_RELEASE` 并出现新的可用锁定卡包后可继续。MainScene 仍复用自己的 `TaskItem` 实例，但隐藏整个 `ProgressBg` 和 `BagBg`，只让 `TaskContent` 在卡片内自动居中显示本地化的“当前阶段的卡包已收集完毕！”。该终态不得修改共享 `TaskItem.prefab` 或 GameScene 结算页实例。
...
- 当前默认 Editor/Demo 构建只开放 `CardBag001~018`，统一由 `GameDefine.IsCardPackAvailableInCurrentBuild` 判定；当前构建的卡包记录、完成数量、首页视觉预加载和列表、直接解锁、首次通关奖励及任务奖励候选均不得包含 019+。已有存档中的 019+ 数据保留但在 Demo 隐藏；定义 `PUFFIES_STEAM_RELEASE` 后解除该内容上限。现有任务奖励权益规则不变：没有 Demo 范围内候选时保持待发，但不得兑现为 019+。
...
- 当前 CardBag010 的 25 片统一命名为 `Piece0101~Piece0125`，只包含第一组，进入关卡时一次给出全部 Piece；对应输出只保留 `Group01` 默认、关卡和贴纸描边。CardBag017 为 `1316 x 1316`、38 片，已完成正式 `PieceGGII` 命名并分为 6 组；对应 6 组描边资源已重新生成。CardBag018 的 196 片按 2026-09-07 手绘区域人工分为 32 组，正式命名为 `Piece0101~Piece3210`，组内按棋盘中心从左到右排列；修改该分组后必须重烘焙 `Group01~32` 三套描边。CardBag022 当前为 `1300 x 1231`、34 片，按当前 Prefab 的正式命名分为 3 组；旧 `Group04~14` 输出已作为过期资源清理。运行时只按 Sprite 原生尺寸同步 Piece 槽位，不得用受 TextureImporter 降采样影响的 `sprite.rect.size` 覆盖 GameBoard 的 Prefab 设计尺寸。
