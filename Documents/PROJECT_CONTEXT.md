...
- 当前默认 Editor/Demo 构建只开放 `CardBag001~018`，统一由 `GameDefine.DemoMaximumBagId=18` 和 `IsCardPackAvailableInCurrentBuild` 判定；卡包记录查询、完成数量、首页视觉预加载和列表、直接解锁、首次通关奖励及任务奖励候选均排除 019 及后续卡包。Admin 的显示全部命令不能突破 Demo 构建上限。后续卡包资源、配置和已有本地进度保留；定义 `PUFFIES_STEAM_RELEASE` 后解除 PackId 上限，但仍只处理存在有效配置的卡包。`CardBag023` 没有 `CardPacks.csv` 配置，继续不进入业务。现有任务奖励权益规则不变：没有当前构建范围内候选时保持待发。
