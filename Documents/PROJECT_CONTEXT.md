...
- MainScene 进入完整卡包开包舞台并开始等待玩家轻点或横划时，或撕开卡包开始转场时，低优先级异步预加载当前 `CardBagNNN` Prefab，随后将 GameScene 加载到 `90%` 待激活状态。撕开卡包至少居中停顿 `0.68s`，并在该静止阶段等待 GameScene 达到可激活状态，最长等待 `5s`；随后将选中卡包 Canvas 临时设为跨场景对象并开放 GameScene 激活。跨场景动画不得在 MainScene 的移交阶段启动；GameScene 完成反序列化和同步初始化、把转场 Canvas 绑定到 GameScene Camera 并调用 `NotifyGameSceneReady` 后，才显式启动卡包下收，稳定两帧后按原参数播放，`ProgressPieces` 同步分开并以实际屏幕中心平均值记录发牌起点；卡包视觉随即销毁，GameScene 直接从该位置执行已扇出碎片的落托盘动画。GameScene 等待该转场最多 `2s`，超时必须设置最终位置、记录发牌起点、释放跨场景 Canvas 并继续入场；MainScene 每次启动时必须清理残留转场实例，避免重复进入时被旧静态状态阻塞。因此 Scene Integration 和 `GameScene.Start()` 的主线程阻塞只能发生在卡包静止阶段，不得重新夹在卡包下收与发牌之间，也不得因转场协程异常而无限隐藏游戏 UI。移交时必须克隆撕口蒙版 Material/Texture，避免 MainScene 卸载缓存资源破坏转场视觉。GameScene 实例化卡包时优先复用 PackId 匹配的预加载 Prefab，预加载失败或直接进入则回退同步 `Resources.Load`；玩法初始化、Collider 精度、托盘目标位置、顺序和 Piece 缩放规则不变。
