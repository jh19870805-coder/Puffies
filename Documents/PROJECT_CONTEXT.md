...
- 音频资源统一平铺在 `Assets/Audios`：背景音乐使用 `BGM_` 前缀，短音效使用 `SFX_` 前缀，名称只使用 ASCII、PascalCase 功能名和必要的两位数字序号。`AudioManager` 是 `DontDestroyOnLoad` 常驻单例，业务层直接向 `PlayMusic/PlaySfx` 传 MP3 文件名；`Assets/Resources/AudioCatalog.asset` 持有全部 AudioClip 引用，Editor 会在音频变动、工程加载及构建前自动扫描同步。MainScene 固定播放 `BGM_MainMenu.mp3`；GameScene 在 `BGM_Gameplay_01~05.mp3` 中首次随机并将稳定文件名保存到当前 SaveSlot 的 SQLite `GameAudioPreferences` 集合，非系列按 PackId、系列按系列链首 PackId 复用，后续进入不得重新随机。BGM 与 SFX 使用独立 AudioSource 音量，`AudioListener.volume` 固定为 `1`，避免二次乘算。
