...
- MainScene 卡包排序分四层：第一层为本游戏进程新发放且仍为 `Unlocked` 的卡包，以及生命周期为 `InProgress` 但第一波尚未完整完成的卡包，按解锁时间倒序；第二层为第一波已完整完成的 `InProgress`，按解锁时间升序；第三层为其他 `Unlocked` 未开始卡包，按解锁时间升序；第四层为 `Completed`，按首次完成时间升序。PackId 是时间并列或无效时的确定性依据。本进程新发放标记不持久化且不会被列表读取消费，同一进程内反复进入首页仍持续置顶，只在进程初始化时清空；重启后未开始卡包恢复第三层正常排序。真实进度优先于新发放标记：第一波未完成的 `InProgress` 依据持久化 Piece 进度归入第一层，第一波完整完成后进入第二层，整包完成后进入第四层。`Completed` 重玩不改变首次完成顺序；每日挑战优先级暂缓实现。
- MainScene 对 `Series` 链执行系列槽位折叠：同系列全部已解锁成员只占一个网格位置，当前最高已解锁 Vol 显示在 `PackCover` 前层，上一已解锁 Vol 使用上一 Vol 真实 PackId 的封面显示在同中心 `PackCover2` 后层，后层等比放大至 `1.1` 倍并按系列稳定旋转 `+10°/-10°`；Vol2 起显示对应 `PackVolN.png`。前后封面各自依据所属卡包进度应用完整、彩色撕开或完成灰色撕开状态。点击系列槽打开编辑器搭建的 `PanelBagVol`，按链顺序展示全部已解锁 Vol，初始居中最高 Vol；拖动或左右按钮切换时使用 `PackLeft/PackCenter/PackRight` 的编辑器位置和缩放插值，松手在 `0.25s` 内吸附，分页圆点和操作按钮随当前 Vol 更新。轮播卡包 Z 轴固定为 `0`；居中卡包继续复用现有开包、继续游戏、重玩确认、拍照和返回逻辑，普通卡包仍进入 `PanelBagSelect`。分页网格按完整六列宽度计算固定左右边距并从 `UpperLeft` 排列，因此满页居中且末页从相同第一列开始。
