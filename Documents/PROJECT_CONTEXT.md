...
- 效果图有明确指定分组时，**指定区域优先于源文件显式组号和默认空间分组**。生成器支持在原尺寸效果图上使用不透明、鲜红色（推荐 `#FF0000`）圈线；圈线应闭合或明确连接图片边界，可共用边界，不应穿过一张完整 Piece 的主体。工具以原图红线区域和 Piece 的有效 Alpha 像素归组，不用外接矩形中心猜测，不会把一个指定组按默认 14 片容量拆开；正式命名仍受 99 组、每组 99 片限制，组内仍从左到右。
...
- 手写组号不依赖 OCR 猜测：首次核对后，在同目录 `CardBagNNN.groups.json` 保存 `version=1`、效果图 `previewSha256`、`gapRadius`、`edgeInset`、`groups` 和可选 `pieceOverrides`。`groups` 每项为 `{number,x,y}`，坐标使用**原图左上角**像素、须落在圈内非红线处，组号从 1 连续。`pieceOverrides` 仅用于已经确认的跨线整片 `{piece,group}`，其中 `piece` 是不含扩展名的源 PNG 文件名；不裁碎图，也不修改源文件名。效果图更换后 SHA-256 校验使旧核对记录失效，需要重新核对，不能只改哈希绕过。
