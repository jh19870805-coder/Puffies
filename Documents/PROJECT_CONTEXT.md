...
- 正式支持 18 种语言：`zh-CN`、`en-US`、`ru-RU`、`es-ES`、`es-419`、`pt-BR`、`pt-PT`、`de-DE`、`ko-KR`、`fr-FR`、`ja-JP`、`tr-TR`、`zh-TW`、`pl-PL`、`it-IT`、`uk-UA`、`vi-VN`、`th-TH`。语言页固定为两列九行，语言名称始终显示各自原生名称；同一语言的地区版本使用紧凑地区标记，当前为 `Español (ES/LATAM)` 与 `Português (BR/PT)`。
...
- 当前语言保存在独立的 `PlayerPrefs/Puffies.Language`，首次启动未保存语言时默认英语 `en-US`，不跟随系统或 Steam 语言；已有有效语言选择继续保留。该设置不属于三份游戏进度，不随切换或删除存档变化。语言切换必须立即更新 `LanName1/2` 选中显隐和当前场景内容，场景加载后继续使用已保存语言。
...
AdminScene 的 `TextLocalPath` 在进入页面时显示“数据存储路径：”，显式换行后显示当前设备 `Application.persistentDataPath` 的绝对路径，即三个 `SaveSlotN` 的共同根目录。路径按平台格式展示，不硬编码盘符、用户名或公司名；程序只更新文本，不覆盖编辑器中的字体、材质、字号、换行和 RectTransform 排版。

AdminScene 左侧代码说明区使用 `TextCode101~118` 预留 18 行，每行沿用编辑器既有的 `代码 : 功能说明` 完整单行格式；运行时代码不得拆分内容，也不得查找、写入或切换右侧 `TextCode201~218`。代码、说明和命令类型由 `AdminScene.sCommandDefinitions` 集中维护，按定义顺序启用左侧已有行，左侧未使用行隐藏。首批记录为：`10001001=显示一键通关按钮`、`10001002=隐藏一键通关按钮`、`10002001=显示所有当前卡包`、`10002002=只显示Demo的前18个卡包`、`10002003=解锁所有卡包（测试）`。场景现有 `InputField` 使用标准 `TMP_InputField` 光标，运行时限制为单行最多 8 位数字；点击 `BtnConfirm` 后先执行并持久化匹配命令，成功才返回 MainScene，无效代码或保存失败时保留 AdminScene。Admin 显隐/范围设置存于当前存档 SQLite 的 `AdminSettings/Runtime`：新存档在 Editor、Development Build 和正式构建中均默认隐藏“一键完成”和“自动拼图”，需要测试时通过 `10001001` 显式开启。`IgnoreBuildCardPackLimit` 默认 `false`，普通 Demo 仍只开放 001~018；显式执行 `10002001` 或 `10002003` 后开启全部配置卡包测试范围，不受 Demo 上限限制。`10002003` 先保存测试范围再解锁全部已配置卡包，只补齐 Locked/缺失记录，保留进行中、完成状态和拼图进度；`10002001` 本身不解锁卡包。`10002002` 关闭测试范围、只显示前 18 包，不删除后续卡包记录；范围保存失败必须回滚两个标记并中止后续解锁。

MainScene 卡包选中页与 GameScene 结算页共用 `Assets/Prefabs/PackPhotoItem.prefab` 和 `CardPackPhoto`。两个场景都必须在各自主 `Canvas` 下保持一个名为 `PackPhotoItem` 的 Prefab 实例；该实例自身 Canvas 固定开启 `Override Sorting` 并使用 `sortingOrder=32000`，每次显示前同步父 Canvas 的 Sorting Layer、恢复独立排序并移到最后一个同级，确保覆盖首页选中卡包、游戏结算层和其他运行时 Canvas。拍照统一生成 `1024x1024` PNG，保存到桌面并命名为 `游戏名-YYYY-MM-DD-BagId.png`，随后在面板 `Photo` 中预览，点击 `BtnOK` 关闭。离屏合成必须先以 `2048x2048 + 4x MSAA` 渲染，再通过双线性采样降至最终 `1024x1024`，避免轻微旋转的棋盘外边缘产生明显阶梯锯齿；不得因此改变最终尺寸、构图或颜色。通用组件负责闪光、离屏渲染、保存、预览、根 Animator 时序和临时纹理释放，场景控制器只传入当前 PackId 并控制各自按钮状态，不得复制第二套拍照实现。离屏照片中的完整拼图不得显示 `GameBoard/BoardTitle` 的游戏内投影，拍照临时副本必须清除其投影材质、UI Shadow 和网格扩边组件，但不得修改关卡 Prefab、GameScene 棋盘或 Piece 投影。全屏白色闪光使用当前主相机下独立的固定 `16:9` Canvas，排序值不得超过 Unity 支持的 `32767`；每次播放前重新绑定当前相机，激活并强制刷新后至少等待一帧再开始透明度动画，确保 MainScene 和 GameScene 首次拍照均可见。闪光与图片生成期间根 Animator 必须停止；预览显示时从第 0 帧单次播放 `PackPhoto`，由美术动画控制 `TaskContent` 显示后消失以及 `BtnOK` 出现，代码只负责在动画完整结束前禁用按钮交互。
...
- 积分任务目标按 `1500 -> 2000 -> 2500 -> 3000` 顺序循环，贴纸任务目标按 `45 -> 60 -> 80` 顺序循环，两类任务使用互不影响的持久化游标。完成卡包任务的目标数量从 `2|3` 随机，指定尺寸从 `S|M` 随机，两项独立选择。
- 正常首次完成以卡包原始基础分开始（XS 600、S 800、M 1000、L 1200、XL 1400、XXL 1600、XXXL 2000），将所有符合条件的百分比加成相加后统一相乘，并向上取整。进入 GameScene 时已经为 `Completed` 的卡包按重玩结算：可得基础分仅为原始基础分的 `10%`，但全部加成仍按原始基础分计算，即 `ReplayFinalScore = Ceil(OriginalBaseScore * 10%) + Ceil(OriginalBaseScore * TotalBonusRate)`；进行中会话的继续游戏不属于重玩。
...
- 首页系列叠放槽位只显示一层原生投影：前层 `PackCover` 使用运行时材质副本将投影 Alpha 设为 `0`，后层 `PackCover` 完整保留美术配置的原生投影。不得创建额外联合阴影对象，不得拉伸封面或使用前后卡包外接矩形模拟整体阴影。两张封面自身纹理、撕口、完成态材质和独立层级不变；`PanelBagVol` 展开后的独立卡包继续保留各自正常投影。
...
| `PanelSave/BtnClose` / `PanelSave/BtnReturn` | 通常只关闭存档弹窗，不改变当前活动档位；若本次已删除当前活动档位，则必须进入 `LoadingScene` 重新加载空档数据，不能继续显示已删除进度的运行时缓存 |
...
- Windows Player 默认使用当前显示器原生分辨率的 `FullScreenWindow` 无边框全屏启动；首次没有本地设置时 `IsWindowed=false`，因此 `PanelSet/ToggleFrame` 默认关闭。首次打开开关时明确以 `1920 x 1080` 切换为 `FullScreenMode.Windowed`；窗口化期间监听客户区宽高，尺寸稳定 `0.5s` 后保存，切回全屏前再立即记录一次，因此用户手动拉伸后的尺寸会在下次窗口化或重启时恢复。关闭开关后必须读取 `Screen.currentResolution` 并以原生宽高和刷新率明确调用 `Screen.SetResolution(..., FullScreenWindow)`，不得保留窗口客户区尺寸或顶部窗口边框。音乐和音效更新只应用音频，不得重复切换显示模式。该尺寸记录仅在 Windows Player 生效，Unity Editor 和其他平台不强制分辨率。
- Windows 窗口客户区尺寸变化后，所有页面必须按 `2560x1440`、`16:9` 设计区域整体等比缩放并立即刷新 Canvas、RectTransform 和 Layout；不得裁切 UI，也不得分别拉伸 X/Y。超宽窗口在左右显示黑边，偏窄或偏高窗口在上下显示黑边。MainScene 重新计算卡包分页并保持当前页；GameScene 不得在检测到尺寸变化的同一帧使用尚未稳定的 Canvas 边界，必须等待最新尺寸稳定两帧，并在没有拖拽或互斥动画的安全时机再次配置视口、刷新布局、重算当前组相机、棋盘、托盘及托盘 Piece；窗口刷新只更新相机有效视口，不得重复初始化根 Canvas，根 Canvas Rect 为零时必须延后布局；托盘 Canvas 边界和回收热区必须相对 `Camera.pixelRect` 换算，不能把黑边计入布局，棋盘重复居中必须在当前位置增量应用中心差；活动中的新手引导提示框、箭头和焦点层随后按新坐标重建。MainScene、GameScene、LoadingScene、RankScene、AchieveScene 和运行时根 Canvas 统一使用居中的固定宽高比相机视口与 `CanvasScaler Expand`；独立的全屏底层相机将视口外区域清为纯黑，避免软件鼠标历史帧残影。
...
- 6 套 fallback SDF 必须保留各自的 Atlas、`_MainTex`、`_TextureWidth/Height`、`_GradientScale` 和 `_WeightNormal/Bold`。`TMP Settings` 的 `Match Material Preset` 保持开启，使 fallback 字形在所有页面继承当前 TMP 文本所用的简体中文材质预设，包括字面颜色、描边颜色、描边宽度、Face Dilate、Underlay 和选中态绿色；不得再用 fallback 内嵌默认材质覆盖页面样式，也不得为韩语或泰语单独硬编码视觉参数。这些 fallback Font Asset 统一按与主字体相同的 `36px` 采样字号、`5px` padding 和 `SDFAA_HINTED` 模式烘焙；不得恢复导入包原有的 `90px` 采样，也不得把中文字体 Atlas 直接赋给其他语言字体。
- `Assets/Scenes/MainScene.unity/PanelLanguage` 使用编辑器内固定结构 `LanguageList/Content` 承载 18 种正式支持语言，不使用 ScrollRect、Viewport 或 RectMask2D。正式语言代码按页面逐行顺序固定为：`zh-CN`、`en-US`、`ru-RU`、`es-ES`、`es-419`、`pt-BR`、`pt-PT`、`de-DE`、`ko-KR`、`fr-FR`、`ja-JP`、`tr-TR`、`zh-TW`、`pl-PL`、`it-IT`、`uk-UA`、`vi-VN`、`th-TH`。LanguageList 尺寸为 `492 x 850`；Content 使用固定 2 列 9 行的 `GridLayoutGroup + ContentSizeFitter`，Grid 单元尺寸为 `234 x 70`、横向间距 `24`、纵向间距 `18`。列表项统一引用 `Assets/Prefabs/LanNameItem.prefab`，根 Button 名称格式为 `LanNameItem_<language-code>`，子节点 `LanName1` 是选中样式、`LanName2` 是常规样式。默认英语选中；当前只完成静态页面，尚未接入点击切换、语言持久化或全局文本翻译。
- `LanNameItem.prefab` 的 `LanName1/2` 必须完整沿用用户在 MainScene 模板中设置的 TMP 字体、共享材质、颜色、字重和对齐；构建列表和后续选择逻辑不得覆盖这些视觉属性。当前两者继续使用 `NotoSansSC-Regular SDF` 及其原共享材质，默认字号统一为 `46`，关闭换行并启用 `16~46` Auto Size，保证长语言名完整单行显示；18 个场景实例不单独覆盖字号、字体或材质。导入的 6 组多语言字体资源与 `TMP Settings.asset` 保持独立，后续需要替换语言字体时须另行确认视觉材质方案。
- `PanelLanguage` 第一次打开时必须先激活面板，再刷新 18 个语言项；刷新后强制重建 `LanguageList/Content` 布局，并对全部 `LanName1/2` 重新应用单行字号适配，保证长语言名在页面首帧显示前已经按真实文本框宽度缩小。不得依赖用户切换一次语言来触发布局刷新。
...
- 托盘内容超宽且已向左滚动时，拿起 Piece 若其左侧队列仍有 Piece 越过托盘可视左边界，则只将左侧队列向右补空位，右侧队列保持不动。右侧仍有 Piece 时，补位距离必须按左侧最后一块与右侧第一块的实际渲染边缘计算，使接口恢复标准托盘间距，不能因最左位置截断而在中间留下额外空隙；拿取队尾时仍限制最左 Piece 不得越过初始左边界。左侧没有越界时继续由右侧队列向左补位。从托盘拿起当时队列最后一片并成功让其离开托盘后，剩余整排还会按当前实际左右渲染边界尽可能对齐托盘右侧标准边距，同时受相同左边界限制。正确吸附、合法桌面放置和桌面组合吸附均确认离开；Piece 回托盘、错误回弹或失焦取消继续恢复拿起前快照。
...
- MainScene 设置以集合/键 `GameSettings/Runtime` 保存在 `AppRecords`：音乐音量、音效音量、窗口模式以及最近一次 Windows 窗口客户区的 `WindowWidth/WindowHeight`。
- 音频资源统一平铺在 `Assets/Audios`：背景音乐使用 `BGM_` 前缀，短音效使用 `SFX_` 前缀，名称只使用 ASCII、PascalCase 功能名和必要的两位数字序号。事件型 SFX 源文件不得保留可感知的前导静音；播放时机正确但听感滞后时，应先检查音频波形，再决定是否调整代码。当前 20 个 SFX 已按各自实际波形裁掉或生成无前导静音版本；后续替换素材时需要重新检查。项目 DSP Buffer 固定为 `512`，不得在未验证设备性能和爆音风险时继续降低。`AudioManager` 是 `DontDestroyOnLoad` 常驻单例，业务层直接向 `PlayMusic/PlaySfx` 传 MP3 文件名；所有场景内及运行时动态创建的 `UnityEngine.UI.Button` 由该单例自动绑定 `SFX_ButtonClick.mp3`，同一帧重复调用会去重，页面代码不需要为新按钮单独补通用点击音效。`Assets/Resources/AudioCatalog.asset` 持有全部 AudioClip 引用，Editor 会在音频变动、工程加载及构建前自动扫描同步。LoadingScene 异步加载 Catalog 并预热首页 BGM 与全部 SFX，最多等待 `10s` 后降级继续；SFX 使用 `DecompressOnLoad + preloadAudioData + loadInBackground`，BGM 使用 `Streaming + loadInBackground`。当前 6 首 BGM 的压缩 MP3 合计约 `10.51 MiB`。MainScene 固定播放 `BGM_MainMenu.mp3`；GameScene 在 `BGM_Gameplay_01~05.mp3` 中首次随机并将稳定文件名保存到当前 SaveSlot 的 SQLite `GameAudioPreferences` 集合，非系列按 PackId、系列按系列链首 PackId 复用，后续进入不得重新随机；选中卡包后即后台准备该曲目，GameScene 激活时复用已确定文件名。BGM 与 SFX 使用独立 AudioSource 音量，`AudioListener.volume` 固定为 `1`，避免二次乘算。
- 首页卡包的有效点击在 `HandlePackageGesture` 只播放 `SFX_CardPackClick.mp3`；普通卡包与系列卡包展开协程不播放 `SFX_PopupTransition.mp3`。列表拖动、锁定卡包或被动画拦截的输入不触发。
- 原 `ClickCard.mp3` 的正式资源名是 `SFX_PopupTransition.mp3`，用于真正弹窗成功打开的时机。当前覆盖 MainScene 菜单、通用确认、设置、语言、可使用、存档、重玩确认，以及 MainScene/GameScene 共用的 `PackPhotoItem` 拍照预览。拍照预览仅在照片生成成功并准备显示时播放。`PanelBagSelect`、`PanelBagVol` 属于卡包选择流程，继续只使用 `SFX_CardPackClick.mp3`；GameScene `RewardPanel` 属于结算页面，不按弹窗播放该音效。
- MainScene 卡包拍照和 GameScene 结算拍照共用 `CardPackPhoto`：`SFX_CameraShutter.mp3` 在白色闪光开始淡入的同一帧播放，每次成功启动拍照流程只播放一次；音效不绑定到拍照按钮回调或拍照预览，以免重复或与实际闪光错位。
- 放大的选中卡包从选择页切换到木纹 `BgGame` 背景时，由公共 `PlayMainToGameBackgroundHandoff` 播放一次 `SFX_CardPackReplayConfirm.mp3`（原“卡包到二次确认界面.mp3”）；完整卡包、彩色撕开继续和灰色撕开重玩共用该时机。重玩确认弹窗显示时不播放此音效。
- `SFX_PieceDeal.mp3`（原“分发碎片.mp3”）在每轮实际发牌开始时只启动一次，通过循环 SFX AudioSource 持续覆盖整组 Piece 错峰飞向托盘的过程，并在最后一片飞行结束时立即停止；首次进入关卡和后续组切换使用相同规则。没有有效 Piece Renderer 时不播放，协程中止或 GameScene 销毁时必须停止，且不得改变碎片动画参数。
- `SFX_PuzzleComplete.mp3` 在整关最后一块正确落下、本次 Piece 全部标记为 `IsPlaced` 后立即播放；不得等待吸附动画、绿色效果或结算面板，也不得在后续 `TryAdvanceGroup` 中重复播放。
- 当前默认 Editor/Demo 构建只开放 `CardBag001~018`，统一由 `GameDefine.DemoMaximumBagId=18` 和 `IsCardPackAvailableInCurrentBuild` 判定；未启用 Admin 测试范围时，卡包记录查询、完成数量、首页视觉预加载和列表、直接解锁、首次通关奖励及任务奖励候选均排除 019 及后续卡包。Admin 的 `10002001/10002003` 可显式解除卡包上限用于测试，所有业务统一使用该测试范围，且不改变 Steam Demo App ID；`10002002` 恢复前 18 包范围。后续卡包资源、配置和已有本地进度保留；定义 `PUFFIES_STEAM_RELEASE` 后默认解除 PackId 上限，但仍只处理存在有效配置的卡包。`CardBag023` 没有 `CardPacks.csv` 配置，继续不进入业务。现有任务奖励权益规则不变：没有当前有效范围内候选时保持待发。
- `SFX_CardPackAppear.mp3`（原“卡包出现声音.mp3”）只在结算返回 MainScene 后，其余非奖励卡包从屏幕下方陆续回到列表槽位期间循环播放；没有实际回位对象时不播放，最后一张到位、流程取消、列表状态清理或 MainScene 销毁时立即停止。该持续音效使用独立循环 SFX AudioSource，跟随音效音量且停止时不得打断其他 `PlayOneShot` 音效；结算页奖励底板入场不再使用该音效。
...
- `Assets/Scripts/Model` 有意保持单层扁平目录。相关纯 C# 类型按以下方式合并：`GameManager` 位于 `GameDefine.cs`，CSV 解析类型位于 `GameConfigRepository.cs`，`JsonLocalStore`、`SqliteLocalStore`、`GameSettingsData` 和 `GameSettingsUtility` 位于 `LocalDataStore.cs`，积分类型和 `GameScoreUtility` 位于 `GameTaskUtility.cs`，`GameFontUtility` 位于 `GameCommonUtility.cs`，音频选择数据和接口位于 `AudioManager.cs`。公开类型名和调用点保持不变。
- Model 当前保留 10 个脚本。`AudioManager`、`AudioCatalog`、`CardPackDataUtility`、`GameTaskUtility`、`GameConfigRepository`、`CardPackPhoto`、`CardPackRewardFlyTransition` 和 `GameDefine` 属于大型、Unity 序列化要求或独立模块，不为减少文件数量继续互相合并。旧 `GameAnimationUtility` 与 `CardFxRuntimeUtility` 已随开包特效一起删除。
...
- 每组完成后等待最后一片的绿色 ADD 滑光和本轮光点回弹全部结束，再将 CardBag/棋盘位置、相机正交尺寸和托盘平滑切换到下一组布局；棋盘主体动画约 `0.72s`，新组 Piece 从棋盘区域用约 `0.38s` 错峰进入托盘。切组动画期间锁定拖拽、提示、“一键完成”和“自动拼图”，普通卡包同样使用该流程。
...
- GameScene 的 `BtnCompleteAllTest` 与 `BtnAutoPuzzleTest` 由同一个 Admin 一键通关显隐设置控制；所有新存档默认隐藏，测试时通过 Admin 代码 `10001001` 开启。`BtnCompleteAllTest` 点击后批量持久化当前 CardBag 全部 Piece 编号、显示完整棋盘并调用正式 `ShowRewardPanel()`；`BtnAutoPuzzleTest` 每次只选择当前托盘顺序中的一片 Piece，用约 `0.36s` 飞入自己的凹槽并复用正常落位、存档、切组和结算流程。两种操作都会产生真实本地测试数据。
...
- RewardPanel 使用三个独立标题文本：`TaskBg2/TaskTitle2` 固定保留编辑器配置的“卡包数”，`TaskTitle21` 依次显示“未使用提示”“关闭关卡描边”“关闭贴纸描边”和“快速完成”等实际生效的加成名称，`TaskTitle22` 同时显示该阶段的实际 `+N`，所有语言都不附加“分/points”等单位。`TaskTitle21` 的编辑器矩形固定为 `x=-50、宽410`，右边缘为 `155`；`TaskTitle22` 固定为 `x=220、宽100`，左边缘为 `170`，两者保留 `15px` 间隔，不得相互覆盖。`N` 使用该阶段累计分数与上一阶段分数的差值，确保全部加分之和与最终分一致。结算初始化和基础分滚动期间三者全部隐藏；每条加成只显示 `TaskTitle21/22`；全部加成结束后隐藏二者并显示 `TaskTitle2`，无加成时在基础分滚动结束后直接显示 `TaskTitle2`。分数滚动统一使用 `1.4x` 速度：基础分配置时长 `1.2s` 的实际滚动约 `0.86s`；每条加成先展示文案 `0.16s`，配置滚分时长 `1.08s` 的实际滚动约 `0.77s`；最终分稳定停留 `0.45s`。`SFX_ScoreIncrease.mp3` 使用可暂停循环通道，每段数字实际上涨时播放或续播，分数停止变化时立即暂停，全部计分结束或 GameScene 销毁时彻底停止。非积分任务进度从最后一次有效滚分的 `25%` 位置开始同步推进，没有加成时并入基础分滚动，不再单独追加一段任务等待。加成名称和分数强制单行并以编辑器字号为上限自动缩小；代码不覆盖三个文本的字体、材质、颜色、对齐和其余 RectTransform 字段，积分计算、任务进度和发包结果不变。
...
- 正确 Piece 吸附并顶回占位错误 Piece 时，已经确认吸附的全部 Piece 必须在错误 Piece 触发托盘重排前先退出 `IsOnTray` 集合；托盘布局不得为即将进入棋盘的 Piece 保留空槽。错误 Piece 的回弹、进入托盘后的红色反馈、`0.5s` 补位、随机顺序和滚动边界继续使用既有规则。
- 从托盘拿起的 Piece 在松手判定回托盘后，必须持续保持拿起时的高 `sortingOrder`，直到自身 `0.5s` 回归补位动画完整结束后才恢复普通托盘层级，避免回程中被其他 Piece 的常驻光点覆盖。补位协程无须移动或被中断时必须同步清理临时高层级；光点继续按各自 Piece 层级自动跟随。
...
- `Assets/UI/CardBags/CardBagNNN` 顶层源 PNG（包头、棋盘和碎片）及 `Previews/CardBagNNN.png` 按原始像素尺寸导入：首次导入、替换图片和生成器配置时，读取原图最长边，仅在当前上限不足时将 `Max Size` 提高到能容纳原图的最小二次幂，并关闭 NPOT 尺寸重采样；足够大的现有上限保持不变。Standalone、Android、iPhone、WebGL 已启用的平台覆盖同样提高不足的上限，不开启原本关闭的覆盖，不修改压缩格式。包头生成必须用原图宽高，不能用导入后可能降采样的 `sprite.rect.size` 作为设计尺寸；更新包头宽高时保持原下沿位置、旋转和缩放。原尺寸批量修正入口为 `CardBagPrefabGeneratorEditor.MatchAllCardBagSourceSizes`，不重新识别 Piece、不重分组、不生成或删除描边。
- `UI/CardBags/Previews/CardBagNNN.png` 是完整拼图和 Piece 定位参考图，不作为运行时 Prefab Sprite；它必须与 `GameBoard.png` 画布尺寸一致。
- 效果图有明确指定分组时，**指定区域优先于源文件显式组号和默认空间分组**。生成器支持在原尺寸效果图上使用不透明红色圈线（推荐 `#FF0000`）：先识别鲜红，存在鲜红轮廓时不扩展原 Mask；未找到时追加 `A >= 128、R >= 96、R >= 2G、R >= 2B` 的深红候选。为避免普通红棕插画误判，无核对记录的深红轮廓长边至少为图短边的 1/3，像素面积低于外接矩形的 10%，同时保留原最少像素/跨度检查；有 `.groups.json` 时沿用原几何限制。圈线应闭合或明确连接图片边界，可共用边界，不应穿过一张完整 Piece 的主体。工具以原图红线区域和 Piece 的有效 Alpha 像素归组，不用外接矩形中心猜测，不会把一个指定组按默认 14 片容量拆开；正式命名仍受 99 组、每组 99 片限制，组内仍从左到右。未识别到圈线时，日志明确说明采用源显式或默认空间分组。
- 圈线有小缺口时按原图短边约 `0.15%`（默认 `1~8px` 半径）有限扩展；不无限加粗或更改源图片。归组有效覆盖不足 50%、区域投票第一名不足 70%、手绘编号未核对、编号种子重复落在同一区域、漏标或轮廓漏到整张图片外围时，生成在保存之前报告具体问题，不静默退回默认分组。普通无标记图继续原算法；没有手绘数字的清晰红圈按区域形成自上而下、首行从右到左的蛇形组序。
- 手写组号不依赖 OCR 猜测：孤立笔画检查同时参考主轮廓跨度，较大手写数字也应提示核对，不能直接当作无编号区域自动排序。首次核对后，在同目录 `CardBagNNN.groups.json` 保存 `version=1`、效果图 `previewSha256`、`gapRadius`、`edgeInset`、`groups` 和可选 `pieceOverrides`。`groups` 每项为 `{number,x,y}`，坐标使用**原图左上角**像素、须落在圈内非红线处，组号从 1 连续。`pieceOverrides` 仅用于已经确认的跨线整片 `{piece,group}`，其中 `piece` 是不含扩展名的源 PNG 文件名；不裁碎图，也不修改源文件名。效果图更换后 SHA-256 校验使旧核对记录失效，需要重新核对，不能只改哈希绕过。
- 分组图若经过压暗、覆盖等处理影响定位，可额外保留同画布的干净 `CardBagNNN.reference.png`，并在分组记录中填写 `referenceSha256`。它只替代定位参考图，不作为分组图或运行时 Sprite；位置算法及原置信度阈值不变，两张图都随 Meta 一起同步。`Update Existing Piece Layouts` 可使用此干净参考，但不验证或改变圈选分组，仍保持原手工层级、组号和材质。
...
- 没有圈选指定分组时，`PieceGGII.png` 或 `PiecesGGII.png` 中的四位 `GGII` 直接成为正式对象编号并作为人工覆盖规则。全部使用标准 `piece_###.png` 时，生成器按位置从上到下分带、每行最多两个空间组，每组最多 14 片；从右上开始，第一行从右到左、第二行从左到右，逐行交替形成蛇形组序，组内始终按中心点从左到右编号。最终 Hierarchy 按 `PieceGGII` 升序创建。标准名与显式正式名不得在同一卡包内混用。
- CardBag005 当前保留手工制作的 19 片、4 个原分组，组序为右上、左上、左下、右下，片数依次 `4/4/4/7`；2026-09-12 仅把原组号 `03/04/01/02` 映射为新 `01/02/03/04` 并重烘焙四组三套描边，组内编号与所有贴图、布局和材质保持不变，不按自动规则重新拆成两组。旧进行中存档的 Piece 编号未迁移，应重新开始该关验收。
...
- 单包生成显式请求烘焙时，只烘焙该包并保留仍存在组号的描边 Meta/GUID，不触发其他卡包烘焙。命令行 `GenerateCardBagFromCommandLine -cardBagId N -bakeCardBagOutlines` 可完成单包生成与三套描边更新；不带此可选参数时仍保持原来的不烘焙行为。
- 同一窗口的 **Update Existing Piece Layouts** 用于切图更新后的局部校准。它复用 Preview/GameBoard 定位算法，通过现有 Piece 的 Sprite 资源路径映射节点，更新 `RectTransform.anchoredPosition` 与 `sizeDelta`，同时将已有 `BoardTitle` 尺寸匹配原图并保持下沿位置；不重建层级、不改变手工分组、Image 参数、影子、旋转缩放或描边资源，也不会自动烘焙描边。更新采用整包事务：源 PNG 与现有 Piece 数量或引用不一致、定位不唯一，或两张有效面积相近的切图在目标位置的高 Alpha 区域重叠达到 `65%` 时，该 Prefab 在保存前失败，避免重复切图覆盖正确布局；面积明显较小且位于大切图内部的独立配件允许更新。
- 卡包可使用 `BoardFixedPieceNN` 表示初始已经显示在棋盘上的固定贴纸；该名称不符合运行时 `PieceGGII` 解析，因此不会进入发牌、拖拽、提示、完成计数或存档。固定贴纸必须是根节点直属 Image，按名称升序放在 `GameBoard` 与正式玩法 Piece 之间。层级工具和 **Update Existing Piece Layouts** 会把它作为源 Piece 保留并更新位置、尺寸，但完整覆盖生成仍只按源文件规则创建正式玩法 Piece。描边烘焙时固定 Piece 的 Alpha 会加入最终拼图区域，并作为首个玩法组开始前已经完成的 `completedMask`；它仍不生成独立 Group。
...
- 当前 CardBag010 的 25 片统一命名为 `Piece0101~Piece0125`，只包含第一组，进入关卡时一次给出全部 Piece；对应输出只保留 `Group01` 默认、关卡和贴纸描边。CardBag015 按 2026-09-08 手绘区域整理：`piece_018` 使用不符合 `PieceGGII` 的独立节点名 `BoardFixedPiece01`，初始即在棋盘显示且不参与发牌、提示、完成计数或存档；其余 33 片对应手绘第 2~8 区，连续重编号为 7 个玩法组 `Piece01xx~Piece07xx`，并已生成 `Group01~07` 三套描边。CardBag017 为 `1316 x 1316`、38 片，已完成正式 `PieceGGII` 命名并分为 6 组；对应 6 组描边资源已重新生成。CardBag022 当前为 `1300 x 1231`、34 片，按当前 Prefab 的正式命名分为 3 组；旧 `Group04~14` 输出已作为过期资源清理。运行时只按 Sprite 原生尺寸同步 Piece 槽位，不得用受 TextureImporter 降采样影响的 `sprite.rect.size` 覆盖 GameBoard 的 Prefab 设计尺寸。
- CardBag013 当前为 **38 片、5 组**、棋盘 `1300 x 1513`，已按 2026-09-13 新效果图的深红圈线及手写 1~5 生成，组片数依次为 `7/10/9/7/5`，对应五组三套描边（15 张 PNG）。核对记录为 `Previews/CardBag013.groups.json`，使用 8px 有限补缝、0px 边缘内缩，无跨线强制归属和额外干净参考图；定位沿用 GameBoard 回退，源图、布局、缩放和材质不变。应重新开始该关验收，不使用旧三组编号的进行中存档。
- 2026-09-21 已交换 `CardBag003 <-> CardBag006` 与 `CardBag004 <-> CardBag008`。当前 003 是原 006 内容：29 张源 Piece、6 组，组片数 `3/3/5/6/6/6`；当前 004 是原 008 内容：33 张源 Piece、6 组；当前 006 是原 003 内容：31 张源 Piece、7 组，组片数 `5/5/5/4/5/2/5`；当前 008 是原 004 内容：31 张源 Piece、5 组。原 003 的 `Series=18` 和手工 `AutoUpdate=0` 随内容迁移到当前 006，因此当前系列链为 `006 -> 018`。003/006 的人工分组记录、干净定位参考图、Preview 哈希和有限补缝参数均随主题资源及 `.meta` 一起移动；009 仍为 4 组、组片数 `6/8/7/15`，使用 10px 有限补缝。玩家进度按 PackId 保存，资源换号不迁移旧开发存档，验收换号卡包时使用新存档或清除对应进度。
- CardBag018 当前为 **192 片、32 组**、棋盘 `2600 x 3920`，已按 2026-09-13 新效果图重新生成 `Piece0101~Piece3209` 和 32 组三套描边（96 张 PNG）。该轮开始前工作区为 192 片/14 自动组，旧文档的 196 片不可作为当前基线。跨线碎片经用户确认：`piece_087` 归第 27 组（`Piece2702`），`piece_108` 归第 25 组（`Piece2509`）；规则保存在 `Previews/CardBag018.groups.json`，其余成员由圈线及 Piece Alpha 归属生成，不在代码硬编码。源 Sprite、全部 Piece/棋盘/包头布局与材质保持不变，Prefab 与原有描边 GUID 保留。修改此分组后必须重烘焙三套描边，并重新开始该关验收，不能使用旧组号的进行中存档。

### 拼图描边渲染

- 拼图描边由 `PuzzleOutlineBakerEditor` 离线生成，并通过 Unity UGUI `Image` 渲染。
- 含 `BoardFixedPieceNN` 的新结构会把固定 Piece 作为默认连接描边的初始完成区域；没有固定 Piece 的旧卡包保持原烘焙路径。
...
- 首页列表分页圆点直接配置在 `MainScene/Canvas/PackageScrollView/PageIndicators`，与 `Viewport` 同级，不随 Content 滚动且不受其软裁切；底部锚点居中 `Y=-32`、容器高 `40`。`DotTemplate/DotNormal|DotSelected` 复用 `PanelBagVol` 的圆点 Sprite/材质/颜色，普通态 `25 x 25`、选中态 `33 x 33`、横向间距 `18`，所有 Image 关闭 Raycast Target。编辑器保留可见模板供美术调整；运行时隐藏模板并按实际活动 `Page_N` 复用圆点，0/1 页隐藏整个容器，超过一页才显示。ScrollRect 滚动、吸附、列表重建和窗口布局变化后按最近页更新选中态，页数统计与吸附共用；系列折叠后按实际列表槽位分页，不按解锁卡包总数直接计算。当前 Demo 最多 18 包、每页 18 个槽位，正常不会显示多页圆点。
...
- 选中态使用独立 `Screen Space - Camera` Canvas，并复制当前列表实例的完整 `PackNode` 显示，目标封面尺寸为 `600 x 680`；外层只负责统一移动、缩放和淡出，内层保持列表当前撕口、背景、装饰碎片、封面、尺寸标签、材质和 Animator 状态。该 Canvas 与选择面板同样绑定 `Main Camera`。背景使用原分辨率、约 `8px` 半径的两遍可分离高斯模糊，按原色、全不透明显示，不叠加黑色或白色蒙版；普通和系列选中页均在截图同一帧读取显示 Canvas 的相机视口，先裁出有效游戏画面、排除窗口黑边，再按有效区域原生像素尺寸模糊，不能将完整窗口截图再次挤入固定比例 Canvas。返回、拍照和重玩确认继续沿用现有流程。高斯 Shader 位于 `Assets/Resources`，运行时 Material 按需创建和释放，缺失时回退为未虚化原色背景。
...
- 撕包完整发生在 MainScene 的 `BgGame` 开包舞台，不能移到 GameScene。横向撕口光效与模型由 Main Camera 渲染；模型使用制作方 `EffectScene001` 保存的基准 `Scale=2.63 / localZ=0`。场景光效始终保留在 `PackObject` 原层级，完整使用编辑器中人工调整的根/子节点 Transform、Start Size、发射参数、材质和排序；运行时代码不得设置其位置、旋转或缩放，也不让其继承模型 Stage 的适配。MainScene 展开卡包 `PackCarousel` 从 `Y=20` 上移到 `Y=140` 后，`fx_chai_w_001` 场景实例根节点同步从 `Y=238.2` 上移到 `Y=358.2`，横向滑光继续对齐实际撕口。模型尺寸和中心只按正面 `mesh_skin_cardPack_NNN` 计算，背面网格不参与初始对齐。
...
- 完整彩包等待拆包时，卡包去除上下各约 `6%` 灰色封边后的内容区上方 `1/3` 为有效撕开区域。滑动可从屏幕任意位置开始和结束，过程中离开卡包不取消；抬起时只要起终点线段与有效区域相交且达到既有最小滑动距离，就执行拆包。有效区域内短点击仍可拆包，区域外短点击无效。
