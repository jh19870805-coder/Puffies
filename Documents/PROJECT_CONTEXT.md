...
- 点击卡包“玩”或确认“重玩”后，选中卡包保持当前位置与尺寸不动；首页背景和 `PackageScrollView` 归入运行时 `MainPageTransitionRoot` 并与虚化背景一起向左滑出，`PanelBagSelect` 向下滑出，`BgGame` 从右侧同步滑入。不得移动 `Screen Space - Camera` 根 Canvas 来实现该交接，避免列表滞留。完整彩包在该背景上播放滑光提示并等待拆包，拆包特效完成后恢复同位置的撕开静态包；彩色进行中撕开包和灰色重玩撕开包跳过拆包，三者随后共用同一跨场景流程。低优先级异步预加载当前 `CardBagNNN` Prefab 并将 GameScene 加载到 `90%` 待激活状态；撕开卡包至少静止 `0.255s`，并在该阶段等待 GameScene 达到可激活状态，最长等待 `5s`，随后将选中卡包 Canvas 临时设为跨场景对象并开放 GameScene 激活。视觉动画使用基础参数的 `1.5` 倍时长，只有该静止等待经过额外缩短；预加载超时、统一两帧稳定和单帧最大推进量不随倍率变化。`CardPackGameEntranceTransition` 只保存跨场景卡包、装饰碎片、材质和位置数据，不得在该 Canvas 自身 MonoBehaviour 上启动决定流程推进的协程。GameScene 完成初始化并绑定 Camera 后，先缓存托盘终点并把当前组真实 Piece 以最终托盘缩放叠放在卡包初始中心；统一稳定两帧后，卡包下落、棋盘与托盘入场、游戏按钮淡入立即在同一帧并行开始，不再为棋盘、按钮或卡包追加独立起步延迟。碎片不得淡入或从下方向上滑入；卡包慢速下落距离按其 RectTransform 实际显示高度计算，完整越过初始碎片区域后再加速掉出屏幕。进入加速段的同一帧隐藏首页装饰碎片，真实 Piece 不经过散开阶段，按托盘顺序从同一个初始位置依次直飞各自终点。转场期间会同时存在 GameScene Canvas 和跨场景卡包 Canvas，因此 GameScene 配置 Canvas、实例化 `CardBagNNN` 或挂载场景 UI 时必须按当前激活场景的根对象查找，禁止使用无场景约束的 `FindObjectOfType<Canvas>()`；否则临时 Canvas 释放时会连带销毁玩法对象。每段视觉转场允许“动画时长 + `1s` 宽限”，异常或超时必须强制进入该段终点并继续玩法；MainScene 每次启动时清理残留实例。移交时必须克隆撕口蒙版 Material/Texture，避免 MainScene 卸载缓存资源破坏转场视觉。GameScene 实例化卡包时优先复用 PackId 匹配的预加载 Prefab，预加载失败或直接进入则回退同步 `Resources.Load`；玩法初始化、Collider 精度、托盘目标位置、顺序和 Piece 缩放规则不变。
