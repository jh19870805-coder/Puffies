...
- `GameScene/BtnTips` 从当前组选择 Piece 编号最小的未完成碎片。目标碎片在托盘原位置左右抖动，棋盘对应 `GrooveRect` 使用 `Resources/Effects/HintDashedOutline.shader` 从 Piece Sprite Alpha 实时生成绿色滚动虚线；拖动目标时暂停抖动，放置失败后恢复，成功放置、切组或结算时清理。只有找到有效目标后才记录本局已使用提示。
