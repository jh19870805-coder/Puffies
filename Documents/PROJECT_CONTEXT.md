...
- 点击结算 `BtnFinish` 后，顶部 `TaskBg2/TaskItem` 先按入场的反向节奏收回屏幕上方，底部 `ImgBagBg/BtnFinish/BtnCamera` 同时收回屏幕下方；顶部 `0.52s`、底部 `0.42s`，不改变透明度、缩放或旋转。已经显示的真实奖励图标在退场前脱离 `ImgBagBg` 并提升到 `RewardPanel` 顶层，保持原屏幕位置，不能随底板退出；提升前必须读取奖励图在 `RewardPanel` 坐标系内的实际中心与显示尺寸，换父后显式恢复 Anchor、Pivot、中心、尺寸、单位缩放和零旋转，不得仅使用 `SetParent(true)` 导致位置被带到屏幕中间。退场全部完成后，`CardPackRewardFlyTransition` 才负责首页切换和奖励卡入列，不属于开包特效。GameScene 最后一帧通过全屏 RenderTexture 覆盖层跨场景保留，截图必须包含静止奖励卡，不得为截图临时隐藏奖励卡；MainScene 列表完成创建和排序后，先缓存真实槽位并把全部列表卡包移到屏幕下方外侧，再用 `0.30s` 直接淡出结算覆盖图显示首页，同时让同位置的飞行副本从透明补到完整，保证奖励卡连续显示。已经分配真实 PackId 的奖励随后从 `BagRewardItem/Canvas/BagCover` 占位图所在槽位，以单张 `0.72s`、多张 `0.12s` 错峰的弧线移动并同步缩放到对应列表尺寸，途中不得提前加载真实封面。落位时把 `BagRewardItem/Canvas/FX_ui_jieSuo_w` 运行时副本继续挂到已经验证可见的目标 `PackCover` 下；按占位卡与目标卡显示尺寸比统一等比缩放，并除以 `PackCover` 相对根 Canvas 的实际继承缩放，从而抵消 `PackNode=0.4`，不得改挂根 Canvas导致粒子不可见。闪光开始 `0.20s` 后先显示真实卡包并刷新 Canvas，飞行占位图继续覆盖 `0.05s` 后再隐藏，保证至少一个渲染帧重叠。循环粒子不阻塞揭晓和其余卡包入场，流程结束时统一清理。全部奖励揭晓后，其余卡包按单卡 `0.44s`、错峰 `0.055s` 从屏幕下方依次滑入。无真实奖励时仍执行跨场景淡出和整列入场；系列后层奖励映射到系列实际列表槽。动画结束或取消时必须释放截图 RenderTexture，并恢复 GridLayout、ScrollRect、列表位置和输入。
