...
- MainScene 环境与 `EffectScene001` 对齐：Ambient Mode 为 Trilight，Sky/Equator/Ground 分别为 `(0.51936734,0.5959181,0.7490196)`、`(0.50146854,0.53454965,0.5566038)`、`(0.2735849,0.25594813,0.22067462)`；使用制作方 Skybox，Directional Light 强度为 `1.3` 并作为 Sun Source；主相机运行时使用与示例一致的正交尺寸 `2.66`，确保依赖相机世界位置的 `Puffies/2_Sided` 反射和高光计算一致。
- MainScene 列表卡包的骨骼初始姿态必须采样制作方 `CardPackOpening` Animator 的第 0 帧，不能在首次采样前禁用 Animator；采样后以速度 `0` 暂停，正式开包时再从第 0 帧重置并播放。
