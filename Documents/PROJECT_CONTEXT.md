...
- MainScene 进入完整卡包开包舞台并开始等待玩家轻点或横划时，或撕开卡包开始转场时，低优先级异步预加载当前 `CardBagNNN` Prefab，随后将 GameScene 加载到 `90%` 待激活状态。撕开卡包至少居中停顿 `0.68s`，并在该静止阶段等待 GameScene 达到可激活状态，最长等待 `5s`；随后将选中卡包 Canvas 临时设为跨场景对象并开放 GameScene 激活。`CardPackGameEntranceTransition` 只保存跨场景卡包、碎片、材质和位置数据，不得在该 Canvas 自身 MonoBehaviour 上启动决定流程推进的协程；GameScene 完成反序列化和同步初始化、把转场 Canvas 绑定到 GameScene Camera 后，由 GameScene 的入场协程逐帧执行转场枚举器。Canvas 若在切场景期间被禁用，播放前必须恢复激活；稳定两帧后按原参数完成卡包下收，`ProgressPieces` 同步分开并以实际屏幕中心平均值记录发牌起点；卡包视觉随即销毁，GameScene 直接从该位置执行已扇出碎片的落托盘动画。转场期间会同时存在 GameScene Canvas 和跨场景卡包 Canvas，因此 GameScene 配置 Canvas、实例化 `CardBagNNN` 或挂载场景 UI 时必须按当前激活场景的根对象查找，禁止使用无场景约束的 `FindObjectOfType<Canvas>()`；否则临时 Canvas 释放时会连带销毁玩法对象。该视觉转场最多允许“动画时长 + `1s` 宽限”，逐帧执行发生异常或超时必须强制落到最终状态、记录发牌点并释放 Canvas，玩法流程不能被视觉动画阻断；阶段日志必须覆盖创建、相机绑定、开始、释放、棋盘/托盘继续入场和整体完成。MainScene 每次启动时必须清理残留转场实例，避免重复进入时被旧静态状态阻塞。因此 Scene Integration 和 `GameScene.Start()` 的主线程阻塞只能发生在卡包静止阶段，不得重新夹在卡包下收与发牌之间，也不得因跨场景对象协程停止而无限隐藏游戏 UI。移交时必须克隆撕口蒙版 Material/Texture，避免 MainScene 卸载缓存资源破坏转场视觉。GameScene 实例化卡包时优先复用 PackId 匹配的预加载 Prefab，预加载失败或直接进入则回退同步 `Resources.Load`；玩法初始化、Collider 精度、托盘目标位置、顺序和 Piece 缩放规则不变。
