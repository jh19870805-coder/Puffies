...
- 完整彩包使用制作方 `Assets/Resources/Effects/CardFx/Animations/test.playable` 驱动拆包、下落、滑光和完成回调，不得再用 Animator、粒子状态或固定时长手写替代。该 Timeline 总长 `7s`，但其 `0~2.633s` 是制作方从 `240x272` 小卡包移动并放大到 `600x680` 的前置阶段；当前游戏进入拆包交互前已经完成同样的选中放大，因此运行时自动读取 Recorded Animation Track 的 `infiniteClip.length`，从约 `2.633s` 的大卡包完成帧开始播放，不能从 `0s` 重复放大。模型在该帧按当前选中卡包的实际中心和高度适配；`Take 001` 为 `3.4667~5.3s`，`fx_chai_w_001` 为 `3.9667~7s`。
- 两个 Animation Track 继续绑定当前模型 Animator；`Image` Control Track 绑定当前静态卡包以保持资源绑定完整，`fx_chai_w_001` 绑定 MainScene `PackObject` 下现有实例，Timeline 自带 `particleRandomSeed=1`。`blur 2~7s` 不属于当前游戏舞台需要的显示内容，只绑定无 Renderer 的运行时代理，不得创建可见 Canvas、Image、`blue.mat` 或任何额外蒙版。
- GameScene 交接点使用 Timeline 内 `Take 001` 起点加真实下落关键帧 `0.800s`，即约 `4.2667s`。真正开放场景激活前暂停 `PlayableDirector`；跨场景对象加载完成后，把 EffectLayer 31 临时加入 GameScene 自身 MainCamera 的 Culling Mask，等待一帧再从同一 Timeline 时间恢复，场景加载阻塞不得推进动画。完整拆包是否跨场景存活只由 Timeline 是否成功启动决定，不得依赖当前组碎片或散点投影是否可用。
- 完整彩包的 `PlayableDirector` 使用 `DirectorWrapMode.Hold`，不得在 Timeline 自然结束后回到 `0s`，因为 Recorded Track 的 `0s` 会把卡包模型两个 Renderer 关闭。播放监控在末帧到达前把 Director 固定到完整时长、Evaluate 并 Pause，随后只标记播放完成并保留整套对象；`PlayableDirector.stopped` 仅作异常漏网兜底。自然完成不释放模型、滑光、blur 代理、临时 Piece、运行时材质或跨场景根对象，也不调用 `Destroy(gameObject)`。不得使用滑光结束、粒子 `IsAlive`、Animator `normalizedTime` 或固定延时提前隐藏、停止或销毁任一部分。准备失败、异常中断或对象被外部销毁时仍可强制清理，但必须先取消完成回调，不能误报自然结束。正式参考是 `EffectScene001` Timeline 绑定的主 Canvas 下实例；开包流程继续复用 MainScene 场景 `Canvas/PackObject` 下已调好的光效实例，不通过 `Resources.Load/Instantiate` 创建第二份，也不覆盖根或子节点 Transform、Start Size、材质、粒子模块和相对排序。
...
- 灰色撕开完成包打开 `PanelReplay` 时只临时隐藏当前选中卡包 Canvas。确认重玩后必须先关闭弹窗并恢复同一份选中视觉，保留其灰色材质、撕口蒙版、尺寸和位置，再清空/创建会话并进入统一转场；禁止从已隐藏的首页槽位重新克隆，因为克隆会继承被关闭的 Image 状态而造成灰色卡包后续动作不可见。
  ...
  - 静态封面切换到 3D 模型时，运行时先采样 Recorded Animation Track 的真实结束帧约 `2.633s`，将此时的大卡包模型正面与当前已经放大的 `600x680` 静态卡包尺寸和中心对齐，再隐藏静态视觉并从同一帧继续 Timeline。不得按 `2s` 的小卡包帧对齐后重复放大，也不得回到 `0s` 重播选择页已经完成的放大阶段。
