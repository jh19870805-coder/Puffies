...
- 彩色撕开和灰色撕开使用相同的跨场景动画节奏：背景交接后不增加额外静止时间；真实 Piece 在卡包移动前以最终 `TrayScale` 和正常 Alpha 创建在卡包中心附近的小范围散点；起飞点为卡包下移约自身显示高度 `72%` 的位置，起飞点前后两段按距离占比分配同一个 `0.345s` 总时长并使用线性位移，最终下移距离按卡包实际顶边到 Canvas 底边动态补足；Piece 以 `0.027s` 错峰和 `0.39s` 单片时长飞向托盘。两者唯一差异是彩色撕开需要把首页展示用 `ProgressPieces` 下收至撕口并隐藏，灰色撕开没有假碎片，不执行该操作。
