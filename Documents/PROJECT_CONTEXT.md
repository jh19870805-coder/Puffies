...
- CardBag003/006/009 的 2026-09-13 新效果图已完成人工编号核对，分别配置为 **7/6/4 组**，预期组片数依次为 003=`5/5/5/4/5/2/5`、006=`3/3/5/6/6/6`、009=`6/8/7/15`。核对记录与当前标注图 SHA-256 绑定；003/006 使用 8px 有限补缝，009 使用 10px 以闭合第 2、3 组之间的较大缺口。三包均保留与对应 GameBoard 同画布尺寸的 `.reference.png` 只用于 Piece 定位，无跨线 `pieceOverrides`；003 工程 Preview 已针对历史 `1299x1171` GameBoard 裁去新图左、上各 1px，原始美术备份不变。实际重新生成后须同步烘焙三套描边并重新开始关卡验收。
