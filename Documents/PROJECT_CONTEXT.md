...
- MainScene 进入完整卡包开包舞台并开始等待玩家轻点或横划时，或撕开卡包开始转场时，低优先级异步预加载当前 `CardBagNNN` Prefab，随后将 GameScene 加载到 `90%` 待激活状态。撕开卡包至少居中停顿 `0.68s`，并在该静止阶段等待 GameScene 达到可激活状态，最长等待 `5s`；随后将选中卡包 Canvas 临时设为跨场景对象并开放 GameScene 激活。GameScene 完成反序列化、同步初始化和两帧稳定后，跨场景卡包才按原参数下收，`ProgressPieces` 同步分开并以实际屏幕中心平均值记录发牌起点；卡包视觉随即销毁，GameScene 直接从该位置执行已扇出碎片的落托盘动画。因此 Scene Integration 和 `GameScene.Start()` 的主线程阻塞只能发生在卡包静止阶段，不得重新夹在卡包下收与发牌之间。移交时必须克隆撕口蒙版 Material/Texture，避免 MainScene 卸载缓存资源破坏转场视觉。GameScene 实例化卡包时优先复用 PackId 匹配的预加载 Prefab，预加载失败或直接进入则回退同步 `Resources.Load`；玩法初始化、Collider 精度、托盘目标位置、顺序和 Piece 缩放规则不变。
