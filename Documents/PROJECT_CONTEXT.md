...
AdminScene 左侧代码说明区使用 `TextCode101~118` 预留 18 行，每行沿用编辑器既有的 `代码 : 功能说明` 完整单行格式；运行时代码不得拆分内容，也不得查找、写入或切换右侧 `TextCode201~218`。代码、说明和命令类型由 `AdminScene.sCommandDefinitions` 集中维护，按定义顺序启用左侧已有行，左侧未使用行隐藏。首批记录为：`10001001=显示一键通关按钮`、`10001002=隐藏一键通关按钮`、`10002001=显示所有当前卡包`、`10002002=只显示Demo的前18个卡包`、`10002003=解锁所有卡包（测试）`。场景现有 `InputField` 使用标准 `TMP_InputField` 光标，运行时限制为单行最多 8 位数字；点击 `BtnConfirm` 后先执行并持久化匹配命令，成功才返回 MainScene，无效代码或保存失败时保留 AdminScene。Admin 显隐/范围设置存于当前存档 SQLite 的 `AdminSettings/Runtime`：一键完成按钮默认仅 Editor/Development 显示，但正式构建可由代码开启。`IgnoreBuildCardPackLimit` 默认 `false`，普通 Demo 仍只开放 001~018；显式执行 `10002001` 或 `10002003` 后开启全部配置卡包测试范围，不受 Demo 上限限制。`10002003` 先保存测试范围再解锁全部已配置卡包，只补齐 Locked/缺失记录，保留进行中、完成状态和拼图进度；`10002001` 本身不解锁卡包。`10002002` 关闭测试范围、只显示前 18 包，不删除后续卡包记录；范围保存失败必须回滚两个标记并中止后续解锁。
...
- GameScene 的 `BtnCompleteAllTest` 与 `BtnAutoPuzzleTest` 由同一个 Admin 一键通关显隐设置控制；默认在 Unity Editor 和 Development Build 中显示，正式非 Development Build 可由 Admin 代码开启。`BtnCompleteAllTest` 点击后批量持久化当前 CardBag 全部 Piece 编号、显示完整棋盘并调用正式 `ShowRewardPanel()`；`BtnAutoPuzzleTest` 每次只选择当前托盘顺序中的一片 Piece，用约 `0.36s` 飞入自己的凹槽并复用正常落位、存档、切组和结算流程。两种操作都会产生真实本地测试数据。
