...
- MainScene 进入完整卡包开包舞台并开始等待玩家轻点或横划时，或撕开卡包开始转场时，低优先级异步预加载当前 `CardBagNNN` Prefab，随后将 GameScene 加载到 `90%` 待激活状态；对应 MainScene 动画结束后只开放场景激活。撕开卡包至少停顿 `0.68s`，并在开始下收前等待 GameScene 达到可激活状态，最长等待 `5s`，因此加载等待不得出现在碎片分开和后续发牌之间；请求激活前不得主动隐藏上一层碎片。GameScene 实例化卡包时优先复用 PackId 匹配的预加载 Prefab，预加载失败或直接进入则回退同步 `Resources.Load`。该流程只前移首次资源读取和场景反序列化，不改变玩法初始化与 Collider 精度。MainScene 撕包结束并进入 GameScene 后，不在游戏页重播卡包模型。撕开卡包退场结束时读取选中 `ProgressPieces` 实际屏幕中心的平均位置并标记碎片已经扇出；其他入口继续使用选中卡包记录位置。托盘目标位置、顺序和 Piece 缩放继续使用 GameScene 已计算结果，不按固定分辨率估算起点。
