...
- `CardPackRewardFlyTransition` 仅负责点击结算完成后的首页切换和奖励卡入列，不属于开包特效。GameScene 到 MainScene 继续使用淡入淡出；MainScene 列表完成创建和排序后，先缓存真实槽位并把全部列表卡包移到屏幕下方外侧。已经分配真实 PackId 的奖励分别复制结算页各自默认 `ImgBag` 图标，从原槽位以单张 `0.72s`、多张 `0.12s` 错峰的弧线移动并同步缩放到对应列表尺寸，途中不得提前加载真实封面。落位后预留 `0.20s` 给后续闪光特效，当前不播放替代特效；预留结束再显示该槽真实卡包完整视觉。全部奖励揭晓后，其余卡包按单卡 `0.44s`、错峰 `0.055s` 从屏幕下方依次滑入。无真实奖励时仍执行淡入淡出和整列入场；系列后层奖励映射到系列实际列表槽。动画结束或取消时必须恢复 GridLayout、ScrollRect、列表位置和输入。
