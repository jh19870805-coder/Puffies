...
- `CardPackRewardFlyTransition` 仅负责结算后新卡包从 RewardPanel 飞到屏幕中央，再飞回 MainScene 对应列表位置，不属于开包特效。GameScene 到 MainScene 继续使用项目现有淡入淡出；奖励卡用 `0.46s` 移到中央并带 `6%` 的轻微尺寸回弹，停留 `0.42s` 后跨场景保留，MainScene 目标飞行单张 `0.64s`，多张奖励按 `0.08s` 错峰并继续使用弧线轨迹。
