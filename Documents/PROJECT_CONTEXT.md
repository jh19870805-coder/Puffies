...
- `LanNameItem.prefab` 的 `LanName1/2` 必须完整沿用用户在 MainScene 模板中设置的 TMP 字体、共享材质、颜色、字重和对齐；构建列表和后续选择逻辑不得覆盖这些视觉属性。当前两者继续使用 `NotoSansSC-Regular SDF` 及其原共享材质，字号上限为 `34`，关闭换行并启用 `16-34` Auto Size，保证长语言名完整单行显示；18 个场景实例不单独覆盖字体或材质。导入的 6 组多语言字体资源与 `TMP Settings.asset` 保持独立，后续需要替换语言字体时须另行确认视觉材质方案。
