...
- RewardPanel 的 `ImgBagBg` 开始上滑 `0.26s` 后启动首张奖励卡，双奖励再错峰 `0.14s` 启动任务奖励飞入。双奖励中的任务奖励直接复用 `BagRewardItemSecondary/Canvas/BagCover`：在任务条 `BagBg/BagIcon` 的实际位置瞬时替换原图标，再用 `0.52s` 直线下移并放大到第二槽，且逐帧读取移动中的槽位终点；原 `BagIcon` 立即隐藏，`BagBg` 绿圈、`BagAddBg` 和 `TextAddNum` 留在原位用 `0.2s` 渐隐。不得复制临时问号图；单奖励仍使用下方槽位原弹出动画。`BtnFinish` 和 `BtnCamera` 在结算首帧只沿 Y 轴放到屏幕下方，不修改编辑器透明度、缩放、旋转或样式；底板和全部奖励卡动画结束后，两按钮同步复用 `ImgBagBg` 的 `0.42s` 位移缓动滑回编辑器终点。无奖励卡包时则在积分和卡包数流程结束后进入。两按钮到位前不可交互。
