...
- MainScene 场景资产逐项保存最新 `EffectScene001` 的 Trilight 三色环境光、制作方 `Skybox.mat`、Subtractive Shadow Color，以及强度 `1.3` 的柔和阴影 Directional Light 参数和方向，并在 Lighting 中将该灯序列化为 `Sun Source`；运行时代码不得重写 RenderSettings、灯光或制作方材质参数。轻点或完成顶部横划后，先沿卡包顶部 `82%` 高度用 `0.42s` 播放从左到右的白色拖尾，再恢复同一实例的 `_001` 原 Animator 播放开包动画和拆包粒子。
