...
- 临时组合按单个整体显示投影：运行时将全部成员 Sprite Alpha 按桌面实际位置合并成一张并集蒙版，成员自身不再分别投影，整体投影直接复用 `IngameCoverShadow02/04` 的既有参数。拿起组合使用 `04`，留在桌面使用 `02`；拖动、错误回弹时整体跟随，合并、拆散、正确入槽、回托盘和切组时重建或释放。不得为组合重新猜测或覆盖材质的颜色、偏移、模糊、扩散参数。
...
- 玩家拿起 Piece 时，为当前鼠标直接选中的主 Piece 创建 `Assets/UI/GameScene/PieceShadow.png` 半透明圆点并随 Piece 移动；圆点默认隐藏，并使用与可见圆点匹配的明确 `CircleCollider2D` 持续检测。只有投影圆点自身与棋盘上 `Alpha > 0` 的已拼 Groove 真实不透明轮廓重叠时才显示，投影离开已拼块立即隐藏，不能以手中 Piece 是否到达棋盘作为触发条件，也不能在桌面、透明凹槽或棋盘空白区域显示。临时组合仍只创建一个圆点，不按成员重复创建。圆点必须作为独立世界对象按当前相机保持资源的原始设计像素尺寸，顶部与 Piece 实际渲染底边固定间隔 `20px`，不能继承 Piece 的棋盘适配缩放而被压小；松手、取消拖拽、窗口失焦和退出场景时立即移除，资源保持原始比例，不重绘。
