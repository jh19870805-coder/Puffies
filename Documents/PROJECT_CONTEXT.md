...
- 撕包完整发生在 MainScene 的 `BgGame` 开包舞台，不能移到 GameScene。横向撕口光效与模型共用 Main Camera 世界舞台；模型使用制作方 `EffectScene001` 保存的基准 `Scale=2.63 / localZ=0`，光效根节点使用固定相对位置 `(0,1,-1.5)` 和原始 `Scale=1`，再由外层 Stage 整体适配选中卡包的屏幕位置和高度。由于粒子使用 `scalingMode=Local`，运行时只将 `line`、`line01`、`glow`、`glowC` 四个光带节点直接放大 `3.5` 倍；`dot`、`dot01`、`glow01` 星形节点保持 Prefab 原始 Transform。不得运行时识别蒙版后单独移动光效，也不得恢复旧独立相机使用的 `Scale=550 / Z=164.02565`。模型尺寸和中心只按正面 `mesh_skin_cardPack_NNN` 计算，背面网格不参与初始对齐。
