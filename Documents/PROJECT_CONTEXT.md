...
- 点击结算 `BtnFinish` 后，顶部 `TaskBg2/TaskItem` 先按入场的反向节奏收回屏幕上方，底部 `ImgBagBg/BtnFinish/BtnCamera` 同时收回屏幕下方；顶部 `0.52s`、底部 `0.42s`，不改变透明度、缩放或旋转。奖励始终以编辑器中组装好的完整 `BagRewardItem` 为单位：单奖励直接复用 GameScene 场景实例，双奖励时才复制第二个完整实例；`Canvas/BagCover` 与 `Canvas/FX_ui_jieSuo_w` 不拆分。退场前整个实例从 `ImgBagBg` 提升到 `RewardPanel` 并保持原显示位置；退场结束后再整体交给 `CardPackRewardFlyTransition` 的持久化 Canvas。两次换父都只能移动和等比缩放完整根节点，并以 `BagCover` 的真实屏幕矩形恢复位置与尺寸，不得重置 Prefab 内部 Canvas 或 `BagCover` 的 Transform。持久化 Canvas 使用 `Screen Space - Camera`，场景切换后重新绑定当前 `Main Camera`；截图淡出阶段可使用覆盖首页的高排序，截图释放后将完整卡包 Canvas 设为 `sortingOrder=1`，确保它稳定显示在 MainScene 根 Canvas `0` 之上，同时保留美术粒子自身 `0~112` 的前后层次。GameScene 最后一帧通过全屏 RenderTexture 覆盖层跨场景保留；MainScene 列表完成创建和排序后，先缓存真实槽位并把全部列表卡包移到屏幕下方外侧，再用 `0.30s` 直接淡出结算覆盖图显示首页。完整 `BagRewardItem` 以单张 `0.72s`、多张 `0.12s` 错峰弧线移动到对应列表尺寸，途中不提前加载真实封面；落位时直接播放实例内已有的 `FX_ui_jieSuo_w`，不复制特效、不换父、不覆盖粒子参数。闪光开始 `0.20s` 后显示真实卡包并刷新 Canvas，原实例封面继续覆盖 `0.05s` 后隐藏。循环粒子不阻塞揭晓和其余卡包入场，流程结束时随完整实例统一清理。全部奖励揭晓后，其余卡包按单卡 `0.44s`、错峰 `0.055s` 从屏幕下方依次滑入。无真实奖励时仍执行跨场景淡出和整列入场；系列后层奖励映射到系列实际列表槽。动画结束或取消时必须释放截图 RenderTexture，并恢复 GridLayout、ScrollRect、列表位置和输入。
