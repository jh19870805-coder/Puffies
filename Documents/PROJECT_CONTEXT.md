...
- CardBag Prefab 保存前会统一校验 `GameBoard`、可选 `BoardTitle` 和全部 `PieceGGII`：Sprite 必须来自同 PackId 的 `Assets/UI/CardBags/CardBagNNN`，Piece 槽数必须等于源 Piece 数，且所有标准源图都必须被引用。保存守卫还会用 `GlobalObjectId` 对比当前 Sprite 对象 GUID 与磁盘 `.meta` GUID，避免 Unity 本地 AssetDatabase 缓存仅返回正确路径却保存错误 GUID。Missing、跨包引用、GUID 不一致或未引用源图会阻止该 CardBag Prefab 保存，不影响其他资源保存。
- Git 拉取、合并或资源导入影响 CardBag Prefab/源目录后，导入监视器会直接解析磁盘 Prefab YAML 的 `m_Sprite` GUID 并与源 PNG `.meta` 对比，在 Console 报告跨设备必然失效的引用。命令行可用 `-executeMethod CardBagPrefabGeneratorEditor.ValidateCardBagReferencesFromCommandLine -cardBagId 19` 校验单包；省略 `-cardBagId` 时校验全部 CardBag，失败时以非零退出码结束。跨设备提交前必须以该磁盘级结果为准，不能只依据当前 Unity Library/AssetDatabase 中的对象路径。
...
- 制作方 Timeline 中骨骼动画先启动，`fx_chai_w_001` 在 `0.5s` 后启动并占用约 `3.033s` 的正式控制轨道。GameScene 在模型 `Take 001` 的 `0.800s` 下落关键帧提前交接，但不得因此清理或停止渲染跨场景特效；真正开放场景激活前必须暂停 Animator、当前正在播放的 ParticleSystem 和逻辑播放时钟，GameScene 自身 MainCamera 绑定 EffectLayer 后等待一帧再从原逻辑时间恢复，场景加载阻塞不得推进动画或清理倒计时。完整拆包是否跨场景存活只由特效是否成功启动决定，不得依赖当前组碎片或散点投影是否可用。跨场景对象加载完成后必须把专用 EffectLayer 31 临时加入新场景 MainCamera 的 Culling Mask，清理时恢复原 Mask。拆包模型只在 Animator 当前状态的短名称确认为 `Take 001` 且 `normalizedTime >= 1` 后隐藏并销毁，不得使用滑光时长、粒子存活或固定延时判断模型结束；模型 Clip 正常约 `1.833s`。滑光使用独立生命周期：完成 `0.5s + 3.033s` 控制时段后只停止发射，等待现存粒子自然结束再释放，不得使用 `StopEmittingAndClear` 截掉尾帧。旧 `1.6s` 主要可见动作上限已取消。正式参考是 `EffectScene001` Timeline 绑定的主 Canvas 下实例，不是场景中另一份世界空间演示实例。开包流程直接复用 MainScene 场景 `Canvas/PackObject` 下已调好的 `fx_chai_w_001` Prefab 实例，不再通过 `Resources.Load/Instantiate` 创建光效，也不覆盖根或子节点 Scale、Start Size、材质、粒子模块和相对排序。主 Canvas 使用 `Screen Space - Camera` 并绑定 Main Camera，与开包模型由同一摄像机渲染；不得以“同一摄像机”为由移动用户配置的父节点。`PackObject` 容器保持激活，内部编辑器参照 `PackItem` 与光效实例默认关闭，播放时只激活光效实例。
...
- 完整彩包的拆包阶段是上述通用跨场景规则的专用前置分支：从 `CardBagNNN.prefab` 选择首个未完成组且只读取尚未拼上的真实 `PieceGGII` Sprite，以场景 `PackObject/fx_chai_w_001` 的实际世界位置作为撕口锚点。撕开滑光在模型播放后 `0.5s` 启动，同一帧全部交接碎片从撕口下方、卡包模型后层以最终显示尺寸出现，并在 `0.32s` 内 EaseOut 向上短跳；跳跃中心高度约为卡包显示高度 `8%`，不做 Alpha 渐显、逐片延迟或 `40% -> 100%` 缩放。跳跃终点直接使用与撕开包真实 Piece 相同的黄金角散点公式及 `20` 倍半径。交接视觉仍沿用首页 `86px * 1.4` 最大边基准，并随卡包从 `240x272` 到 `600x680` 同比例放大，展开状态最大边约 `301px`。
- `Take 001` 到达 `0.800s` 下落关键帧后，MainScene 保存交接碎片散点中心，隐藏并销毁选中静态卡包内容，不恢复第二个撕开静态包、不执行卡包下落，也不创建 `CardPackGameEntranceTransition`。GameScene 当前组真实 Piece 从相同中心用 `GameDefine.CalculatePieceDealScatterOffset` 重建同一套 `20` 倍散点，以最终 `TrayScale`、`0.027s` 错峰和 `0.39s` 单片时长飞向托盘；棋盘、托盘和按钮入场参数与彩色撕开一致。制作方模型和滑光继续跨场景播放完整时序后再清理。完整彩包已经完成模型撕开，因此不重复等待卡包下落起飞点。彩色进行中包与灰色重玩包继续使用可见卡包下落到 `72%` 起飞点后的发牌规则。
