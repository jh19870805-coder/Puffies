...
- 6 套 fallback SDF 的内嵌材质必须与现有 `NotoSansSC-Regular SDF` 保持同类视觉样式：统一使用 `TMP SDF Mobile`、白色描边和 Underlay 参数，但每套字体必须保留自己的 `_MainTex`、`_TextureWidth/Height`、`_GradientScale` 和 `_WeightNormal/Bold`。`TMP Settings` 的 `Match Material Preset` 保持关闭，由各 fallback 使用自身正确的 Atlas 材质；通用参数为 `_OutlineWidth 0.45`、`_FaceDilate 0.2`，KR 因原字体韩文字面偏细使用 `_FaceDilate 0.35`。这些 fallback Font Asset 统一按与主字体相同的 `36px` 采样字号、`5px` padding 和 `SDFAA_HINTED` 模式烘焙；不得恢复导入包原有的 `90px` 采样，也不得把中文字体 Atlas 直接赋给其他语言字体。
