...
- 可见贴纸使用 `UI/GameScene/PieceLight1.png` 到 `PieceLight4.png` 显示不规则暖白亮光。托盘待拼 Piece 与棋盘已拼 Image 各自确定性分布两个亮斑，以低强度、不同相位和约 `1.55~2.35s` 周期持续呼吸并轻微移动；亮斑通过 SpriteMask 或 UGUI Alpha Mask 限制在贴纸真实轮廓内。正确放置 Piece 后仍先从松手位置以三次方减速曲线吸附到凹槽中心，固定 `0.12s`；抵达后当前块继续播放原有约 `0.52s` 的绿色斜向光带，同时用四个不规则亮斑增强，最多六块实际相邻的已拼贴纸各用两个亮斑按距离延迟约 `0.07~0.23s` 向外传播，完整传播约 `0.72s`。绿色光带只作用于当前块，新亮斑只向相邻已拼块传播，不照亮未拼槽位或棋盘背景；两层都结束后才解除落位锁定并继续切组或结算。
