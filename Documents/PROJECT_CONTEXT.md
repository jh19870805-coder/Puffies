...
- `PieceGGII.png` 或 `PiecesGGII.png` 中的四位 `GGII` 直接成为正式对象编号并作为人工覆盖规则。全部使用标准 `piece_###.png` 时，生成器按位置从上到下分带、每行最多两个空间组，每组最多 14 片；从右上开始，第一行从右到左、第二行从左到右，逐行交替形成蛇形组序，组内始终按中心点从左到右编号。最终 Hierarchy 按 `PieceGGII` 升序创建。标准名与显式正式名不得在同一卡包内混用。
...
- 当前 CardBag010 的 25 片统一命名为 `Piece0101~Piece0125`，只包含第一组，进入关卡时一次给出全部 Piece；对应输出只保留 `Group01` 默认、关卡和贴纸描边。CardBag015 按 2026-09-08 手绘区域整理：`piece_018` 使用不符合 `PieceGGII` 的独立节点名 `BoardFixedPiece01`，初始即在棋盘显示且不参与发牌、提示、完成计数或存档；其余 33 片对应手绘第 2~8 区，连续重编号为 7 个玩法组 `Piece01xx~Piece07xx`，并已生成 `Group01~07` 三套描边。CardBag017 为 `1316 x 1316`、38 片，已完成正式 `PieceGGII` 命名并分为 6 组；对应 6 组描边资源已重新生成。CardBag018 的 196 片按 2026-09-07 手绘区域人工分为 32 组，正式命名为 `Piece0101~Piece3210`，组内按棋盘中心从左到右排列；修改该分组后必须重烘焙 `Group01~32` 三套描边。CardBag022 当前为 `1300 x 1231`、34 片，按当前 Prefab 的正式命名分为 3 组；旧 `Group04~14` 输出已作为过期资源清理。运行时只按 Sprite 原生尺寸同步 Piece 槽位，不得用受 TextureImporter 降采样影响的 `sprite.rect.size` 覆盖 GameBoard 的 Prefab 设计尺寸。
