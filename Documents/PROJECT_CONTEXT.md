...
- 卡包可使用 `BoardFixedPieceNN` 表示初始已经显示在棋盘上的固定贴纸；该名称不符合运行时 `PieceGGII` 解析，因此不会进入发牌、拖拽、提示、完成计数或存档。固定贴纸必须是根节点直属 Image，按名称升序放在 `GameBoard` 与正式玩法 Piece 之间。层级工具和 **Update Existing Piece Layouts** 会把它作为源 Piece 保留并更新位置、尺寸，但完整覆盖生成仍只按源文件规则创建正式玩法 Piece。
