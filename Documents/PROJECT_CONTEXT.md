...
- 玩家拿起 Piece 时，在当前鼠标直接选中的主 Piece 下方显示 `Assets/UI/GameScene/PieceShadow.png` 半透明圆点并随 Piece 移动；临时组合仍只显示一个圆点，不按成员重复创建。圆点必须作为独立世界对象按当前相机保持资源的原始设计像素尺寸，顶部与 Piece 实际渲染底边固定间隔 `20px`，不能继承 Piece 的棋盘适配缩放而被压小；松手、取消拖拽、窗口失焦和退出场景时立即隐藏，资源保持原始比例，不重绘。
