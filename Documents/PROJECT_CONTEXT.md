...
- `CardPacks.csv/BoardScale` 使用 invariant-culture 浮点数且必须大于零。GameScene 将其乘到当前 CardBag 根节点，使棋盘、槽位、描边和吸附坐标统一缩放。托盘 Scale 与棋盘 Scale 独立：托盘按 Sprite 原始设计高度和托盘 `90%` 上限计算；Piece 离开托盘后，拖拽、桌面、错误棋盘位置、占用探针和吸附动画统一使用棋盘目标 Scale。棋盘目标先按当前 `SpriteRenderer` 屏幕包围盒与目标凹槽屏幕矩形校准，再以最大 X/Y 轴等比限制到 `1`，任何轴都不得放大，且每次拿起时刷新。未正确吸附的 Piece 保持该棋盘目标 Scale，并只允许完整渲染边界落在棋盘内未被 Alpha 大于 0 的已拼 Piece 占用的位置、完整落在棋盘左右两侧的桌面，或在棋盘左右边界之间完整落入棋盘底边与托盘原始顶部之间的桌面空间；新增下方区域使用托盘缓存的原始屏幕顶部作为下边界，托盘收起后也不得侵入其原始高度。任何横跨棋盘边框、位于棋盘正上方、侵入托盘原始区域或与已拼内容实际轮廓重叠的位置均回弹。成功吸附后立即用 Prefab 对应原始 `Image` 替代拖拽 `SpriteRenderer`，确保已放置 Piece 与棋盘在同一 Canvas 层级共同缩放，接缝不随 `BoardScale` 放大。
