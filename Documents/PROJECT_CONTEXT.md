...
- 正常清理只能从 `PlayableDirector.stopped` 的完整 Timeline 结束回调进入；回调统一释放模型、滑光、blur、临时 Piece、运行时材质和跨场景根对象。不得使用滑光结束、粒子 `IsAlive`、Animator `normalizedTime` 或固定延时提前隐藏、停止或销毁任一部分。异常中断可以强制清理，但必须先取消完成回调，不能误报自然结束。正式参考是 `EffectScene001` Timeline 绑定的主 Canvas 下实例；开包流程继续复用 MainScene 场景 `Canvas/PackObject` 下已调好的光效实例，不通过 `Resources.Load/Instantiate` 创建第二份，也不覆盖根或子节点 Transform、Start Size、材质、粒子模块和相对排序。
