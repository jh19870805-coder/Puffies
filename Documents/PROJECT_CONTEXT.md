...
- `CardPacks.csv/BoardScale` 使用 invariant-culture 浮点数且必须大于零。GameScene 将其乘到当前 CardBag 根节点，使棋盘、槽位、描边和吸附坐标统一缩放。Piece 使用三套互不覆盖的 Scale：`TrayScale` 按 Sprite 原始设计高度和托盘 `90%` 上限计算，最大为 `1`，创建、拿起前快照、回收和托盘布局都会按最大轴统一等比校验；`DragScale` 在 Piece Renderer 创建时保存资源原始 `localScale`，从托盘拿起、桌面放置和桌面错误回弹始终使用该值；`BoardScale` 才按对应凹槽屏幕矩形计算，只供凹槽占用探针和正确吸附动画使用。拿起不得重新计算或覆盖 `DragScale`，也不得把 `BoardScale` 当成拿起尺寸；任何直接回收、错误回弹、被其他 Piece 顶回或失焦回收只要最终回到托盘，都必须恢复拿起前保存的 `TrayScale`，不能保留或写回另两套 Scale。未正确吸附的 Piece 使用资源原始 `DragScale`，并只允许完整渲染边界落在棋盘内未被 Alpha 大于 0 的已拼 Piece 占用的位置、完整落在棋盘左右两侧的桌面，或在棋盘左右边界之间完整落入棋盘底边与托盘原始顶部之间的桌面空间；新增下方区域使用托盘缓存的原始屏幕顶部作为下边界，托盘收起后也不得侵入其原始高度。任何横跨棋盘边框、位于棋盘正上方、侵入托盘原始区域或与已拼内容实际轮廓重叠的位置均回弹。成功吸附时 SpriteRenderer 从 `DragScale` 缓动到 `BoardScale`，随后立即用 Prefab 对应原始 `Image` 替代，确保已放置 Piece 与棋盘在同一 Canvas 层级共同缩放。
...
- 每片可见贴纸只显示一个不规则暖白光点。运行时根据 Piece 的原生宽高和目标光点宽高比，从 `UI/GameScene/PieceLight1.png` 到 `PieceLight4.png` 中选择形状最合适的一张，并在 Piece 尺寸限制内做受控 X/Y 缩放；偏高或短小 Piece 使用紧凑光点，真正偏宽 Piece 使用长弧形光点。位置优先读取 Sprite Physics Shape 的左上极值并向轮廓内部收回，缺少轮廓时回退固定左上位置；托盘 SpriteRenderer 与棋盘 UGUI 共用同一形状、旋转、缩放和归一化位置，并分别通过 SpriteMask 和 Alpha Mask 裁切在 Piece 真实轮廓内。正确放置 Piece 后仍先用固定 `0.12s` 吸附到凹槽中心；抵达并提交棋盘后立即允许拿取托盘下一块，同时当前块继续播放原有约 `0.52s` 的绿色斜向 ADD 光带。当前块和最多六块实际相邻的已拼 Piece 按原 `0.07~0.23s` 错峰触发光点回弹：光点横向 8 段网格的两端固定，中段沿受力方向推出并加厚拉伸，再小幅反向回弹并恢复初始形状和固定位置，不累计位移。新动画从当前形变接管同一光点，旧动画停止写入。绿色光带的 Shader、范围、颜色和时长不变；多个落位反馈可以并发，但切组或结算必须等待全部反馈结束后只触发一次。
