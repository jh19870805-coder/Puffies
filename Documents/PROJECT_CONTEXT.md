...
- 默认 Demo 的 `CardBag001~018` 全部进入 `Unlocked`、`InProgress` 或 `Completed` 后，任务系统对业务返回无活动任务，不再向玩家提供新任务；原 JSON 任务状态保留，定义 `PUFFIES_STEAM_RELEASE` 并出现新的可用锁定卡包后可继续。MainScene 仍复用自己的 `TaskItem` 实例，但隐藏整个 `ProgressBg` 和 `BagBg`，只让 `TaskContent` 在卡片内自动居中显示本地化的“已获得全部卡包”。该终态不得修改共享 `TaskItem.prefab` 或 GameScene 结算页实例。
