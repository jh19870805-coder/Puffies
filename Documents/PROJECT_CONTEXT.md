...
- 系列槽进入 `PanelBagVol` 时，主卡继续使用现有 `0.4s` 弹起放大。列表后层对应的真实 Vol 卡在同一阶段保持位于主卡下方和中心对齐，并在前 `35%` 时间内同时将列表叠放 Z 轴旋转快速归零、将缩放调整到左侧卡位最终值；主卡完全展开并经过现有 `0.15s` 停顿后，后层卡保持该尺寸不变，只沿 X 轴从中心滑向左卡位。不得让后层提前横移，也不得在滑出阶段继续缩放。
