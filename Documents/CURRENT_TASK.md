...
- 任务：调整新手引导第一步提示框布局
- 状态：代码完成并通过 C# 编译，等待 Unity Play Mode 视觉回归
- 更新时间：2026-08-09
...
1. 在 Unity Play Mode 进入 CardBag001，对照用户红框确认第一步提示框位于棋盘左上，文字最多两行且完整显示。
2. 确认第一、二阶段不再叠加第二层黑色遮罩，第一阶段移动箭头等比缩小 30%，第三阶段提示框箭头未受影响。
3. 分别关闭和打开高对比度进入同一卡包，确认描边为 `#3f423e` 和 `#b1d702`，默认、关卡、贴纸三种描边模式一致。
4. 继续回归 CardBag022 蛇形组序、提示描边和一键完成。
