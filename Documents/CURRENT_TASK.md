...
- 运行时虚化截图使用纯白不染色、Alpha `0.45`，让下方原始首页透出 `55%`。相比首轮无黑层时的 `0.6`，降低虚化图占比可减少其亮色区域扩散形成的白雾，同时不引入黑色灰调。
- 进入开包舞台时，虚化截图从 `0.45` 按原转场进度平滑淡出，不会先跳回全不透明；降采样层级、虚化强度、前景排序和选中卡包动画均未修改。
