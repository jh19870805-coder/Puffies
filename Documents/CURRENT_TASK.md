...
- 增加开包 Stage 对齐诊断日志，输出 Game View 分辨率、选中卡包目标像素高度、首帧正面 Bounds、Stage 缩放和屏幕中心。模型继续按选中静态卡包尺寸适配；对比参考画面后，直接将 `fx_chai_w_001` 的 `line`、`line01`、`glow`、`glowC` 四个光带节点按长度 `7.0x`、高度 `1.4x` 非等比缩放，不放大卡包模型、共同 Stage 或光效根节点。
...
- 待在 Play Mode 确认光带节点按 `7.0x × 1.4x` 非等比缩放后更长、更窄，星星保持 Prefab 原始尺寸与分布，且卡包模型尺寸、Stage 和撕口位置没有变化。
...
- 按最新参考反馈将四个光带节点由等比 `3.5x` 改为长度 `7.0x`、高度 `1.4x` 的非等比缩放；两个 C# 工程再次编译通过，均为 `0` 警告、`0` 错误，星形节点和特效资源未修改。
...
4. 继续进入 `BgGame` 并轻点/横划卡包，确认卡包模型保持原适配尺寸、四个光带节点按 `7.0x × 1.4x` 显示为细长光带、星形粒子保持原尺寸，撕包时只存在一层完整卡包，且关闭背面网格后上半包撕裂动画没有空白；随后确认进入 GameScene 时序。
