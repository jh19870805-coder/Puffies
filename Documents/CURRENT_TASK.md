...
- 新增通用 `HintDashedOutlineGraphic` UI Mesh 组件，从目标 Piece Sprite Physics Shape 获取有序轮廓，并通过时间相位生成持续滚动的绿色虚线。
- 虚线对象运行时复制对应 `GrooveRect` 的布局并放到同级最上方，不修改 CardBag Prefab，也不需要新增或重新烘焙提示蒙版。
- 原 Shader 按角度或局部切线分段都会让不规则轮廓上的实际线段长度不一致；现改为沿轮廓累计真实距离，以 `8` 像素周期、`0.7` 填充比例生成固定 `5.6` 像素实线段，并将闭合轮廓余量均摊到间隔中，避免首尾出现半截线。
...
- 提示轮廓由通用 UI Mesh 组件读取 Sprite Physics Shape，不为每个卡包增加美术资源。
