...
- 用户意图：解决 003、006、009 使用新效果图制作关卡时的 `red labels or small separate marks need review` 报错，并按效果图手写编号生成真实分组。
- 定位：该报错是生成器识别到红色圈线及孤立手写数字后主动阻止 OCR 猜号，不是 Piece 定位错误。人工核对确认 003 中央标记为第 7 组，因此 003 为 7 组；006 为 6 组；009 为 4 组。
- 修改：新增三份与当前标注图 SHA-256 绑定的 `CardBagNNN.groups.json` 及 Meta。003/006 使用 `gapRadius=8`；009 的第 2、3 组分隔线缺口更大，使用 `gapRadius=10`。三包均使用与实际 GameBoard 同尺寸的 `CardBagNNN.reference.png` 做纯定位参考并记录独立 SHA-256，避免压暗和红线影响像素定位；不修改源 Piece、生成器代码或运行时逻辑。003 新标注图比历史 GameBoard 宽、高各 1px，经像素相关性核对后仅将工程 Preview 裁去左、上各 1px，原始美术备份保持不变。
- 核对：按现有 Prefab 中每片 Piece 的真实 Rect、源 Sprite GUID 和 Alpha 像素投票验证，所有 Piece 的有效覆盖率及第一候选比例均达到生成器阈值，无跨线歧义，不需要 `pieceOverrides`。预期组片数：003=`5/5/5/4/5/2/5`（31 片），006=`3/3/5/6/6/6`（29 片），009=`6/8/7/15`（36 片）。
- 修改文件：`Assets/UI/CardBags/Previews/CardBag003|006|009.groups.json`、对应 Meta、三张 `.reference.png` 及对应 Meta、任务记录和项目上下文；保留工作区中用户替换效果图、PackIcon Meta 和美术目录改名等既有修改。
- 验证：三张标注图和三张定位参考图的配置哈希逐项一致，且分别与对应 GameBoard 画布尺寸一致；种子按生成器同阈值、同四连通、同方形膨胀规则验证，分别落入 `7/6/4` 个唯一封闭区域。Piece Alpha 归属共 `31/29/36` 片，无低于 50% 有效覆盖或 70% 第一候选的项目。当前 Unity 因未重新获得焦点尚未导入新增资源，因此未执行覆盖 Prefab 和描边烘焙。
- 下一步：切回当前 Unity 等待 Asset Refresh 完成后，在卡包工具中分别生成 003、006、009并烘焙描边；核对日志组片数与上述预期一致，再检查三包 Prefab、三套描边和 Unity Console。分组变化后使用重新开始的关卡验收，不沿用旧组号的进行中存档。

## 2026-09-13 深红分组圈线与 CardBag013 指定五组

- 状态：生成器修复、013 五组生成及描边烘焙完成，Unity 内 21 项测试通过，待重新开始该关进行游戏内验收。
- 用户意图：013 按新效果图手写 `1~5` 的区域和顺序生成，不采用默认三组；同类深红圈线应由通用工具识别。
- 定位：013 圈线主要为 `RGB(179,42,0)`，旧条件 `R > 220` 导致标记像素数为 0，38 片被自动分为 `13/13/12` 三组。
- 设计与边界：鲜红圈线优先，找到后保留原识别 Mask；仅未找到鲜红圈线时按相对红色占比追加深红候选。无核对记录的深红轮廓须满足更严格的长边和细线占比限制，避免红棕插画误判。较大手写编号也必须核对，不猜测 OCR。013 的五个编号通过与新图 SHA-256 绑定的 `.groups.json` 保存，不在代码硬编码 PackId 或 Piece 分组；跨线歧义先报告，不降低原有 70% 归属和 50% 覆盖阈值。只重新生成 013 及对应描边，不改原图、其他卡包、运行时逻辑或玩家存档。
- 工作区：用户已替换 003/006/009/013 的效果图、重生成 013 并删除旧描边；保留所有现有改动。其他新图仅用于识别回归检查，不生成。
- 已完成：核对 `CardBag013.groups.json` 的 1~5 区域种子，采用 8px 有限补缝、0px 边缘内缩，无需跨线覆盖或额外干净参考图。实际生成 38 片、5 组，组片数依次为 `7/10/9/7/5`，三套描边合计 15 张；烘焙结果 `assigned=7475 / unassigned=0 / ambiguous=0 / scale=1`。原图保持不变。
- 验证：在已打开的 Unity Editor 内执行 21 项 NUnit 用例，全部通过；这是临时验证器调用测试方法及断言，不是 TestRunner XML 流程。扫描 23 张现有效果图，18 张普通图无新增误判，013/018 正常识别；新 003/006/009 识别到圈线并明确提示需核对手写组号，未生成这三包。013 生成前后按源 Sprite 对比全部 Image 的贴图引用、位置、尺寸、缩放、旋转、锚点、Pivot、颜色、材质、父级和组件类型均一致，38 片正式名称与核对分区一致，15 张描边 Sprite 全部加载成功。已目视检查实际 Piece Alpha 上色的五组核对图。
- 修改文件：`CardBagPrefabGroupingEditor.cs`、对应测试、013 的 `.groups.json` 及 Meta、013 Prefab 和五组三套描边、任务记录与项目上下文。用户已删除的旧六组描边只重建当前所需的 1~5 组，不恢复旧 Group06。源码 `git diff --check` 通过；不为 Unity 原生 YAML 的空字段尾空格重写资源。
- 收尾：一次性 Unity 验证脚本及 Meta 已移除，现有 Unity 于 18:23:50 成功重新编译并重载程序集，编译输入不再包含临时验证器。本机报告保留在忽略的 `Logs/DeepRedGrouping-generate.txt` 和分组核对图中。未进入 Play Mode，未保存用户当前场景，未提交或推送。
- 下一步：在游戏中重新开始 CardBag013，按效果图的 1~5 顺序检查 `7/10/9/7/5` 片及描边；不要使用旧三组编号的进行中存档验收，不自动删除或迁移存档。013 无需重复生成；003/006/009 若后续处理，应先核对各自的新编号区域并建立分组记录。
...
- 用户意图：点开卡包后，背景首页列表不能变窄或向中间收缩；不修改卡包尺寸、列表间距、呼吸动画、开包流程或虚化颜色和强度。
- 定位：视频固定 UI 特征对齐测得背景横向约为原图的 `95.05%`，纵向约为 `99.99%`。`CaptureBagSelectBackdrop` 截取整个窗口，再将完整纹理铺入固定比例的相机 Canvas，未排除有效游戏视口之外的区域；普通卡包与系列卡包共用此入口。
- 设计：在截图同一帧记录显示 Canvas 的相机视口，先按其归一化范围裁出有效游戏画面，再以该区域原生像素尺寸执行现有两遍 `8px` 高斯模糊。输出继续铺满同一相机 Canvas；不修改相机、UI 布局、Shader、美术资源或玩家存档。
- 修改：`MainScene.CaptureBagSelectBackdrop` 在帧末截图前读取 `mBagSelectOverlayCanvas.worldCamera.rect`；临时与最终纹理按截图尺寸乘该视口比例分配，首次 `Graphics.Blit` 使用相同视口的 `size/position` 裁切。两遍高斯处理、颜色空间转换、显隐、释放以及缺少 Shader 时的回退均保留；每次打开重新读取视口，缺少相机时使用完整截图。
- 验证：当前已打开的 Unity `2022.3.62f2c1` 成功编译 Runtime/Editor 程序集。通过 Direct3D11 执行现有高斯 Shader 的 6 组隔离 GPU 验证：`1024x576` 全屏、`1280x576` 左右黑边、`1024x768` 上下黑边、`576x1024` 竖向窗口、自定义偏移视口、恢复全屏；每组核对 25 个 UV 标记点，最大通道误差 `0.001960814`，符合 8 位纹理量化误差。报告位于本机忽略的 `Logs/BagSelectBackdropValidation.txt`。这是合成标记图的裁切/模糊验证，不是完整首页 Play Mode 回归；未进入游戏、修改存档或保存场景。
- 修改文件：`Assets/Scripts/Controller/MainScene.cs`、本任务记录与 `Documents/PROJECT_CONTEXT.md`；一次性编辑器验证器和 Meta 已移除，不保留额外工具菜单。普通/系列选中共用修复入口，窗口调整继续由原有同相机 Canvas 适配；不修改卡包 Prefab、动画、列表排版和 Shader。
- 下一步：在原视频对应窗口尺寸反复点开、返回普通卡包和系列卡包，检查背景列表、任务框及两侧按钮不再向中间收缩；再分别拉宽/拉窄窗口验证。尚未提交或推送。

## 2026-09-13 效果图指定分组与 CardBag018 重新生成

- 状态：018 已按用户确认的归属重新生成，192 片/32 组与 96 张描边验证通过；待重新开始该关进行游戏内验收。
- 用户意图：018 使用新效果图圈出的分组；以后任意卡包带明确圈选分组时，生成器优先采用指定区域，不再套用默认空间分组。
- 需求与边界：保留原图、源 Sprite 和原像素定位精度；无标记图继续原生成规则；标记不闭合、碎片归属有歧义时明确提示，不静默退回自动分组。组内仍从左到右编号，显式组不受默认每组 14 片限制，但遵循 PieceGGII 上限。
- 设计：将圈线区域提取与现有 Piece 定位分离；按 Piece 不透明区域判断所属圈选区。018 的手写数字人工核对后保存为与效果图内容绑定的分组顺序数据，不在生成器硬编码 PackId 或源碎片编号；图片更换后旧核对数据不得继续套用。
- 实际资源：本次开始前源目录和 Prefab 都是 **192 片、14 个自动组**，旧文档中的 196 片/32 组不是当时工作区的实际状态；现已依据新效果图的 32 个编号区域重新生成。图被压暗，`GameBoard` 透明孔内也没有可用的完整贴纸 RGB，因此从 Git HEAD 的原效果图保留一份 `CardBag018.reference.png`，只用于原算法定位；用户新图保持原样用于分组。
- 已完成：新增 `CardBagPrefabGroupingEditor.cs`，优先采用红色圈选区域，按 Alpha >= 200 的有效像素投票归组，组内从左到右。区域投票第一名不足 70%、有效覆盖不足 50%、组号重复/漏标、圈线未封口等在保存前报告，不修改已有 Prefab。普通效果图和原像素匹配阈值不变。`Update Existing Piece Layouts` 只解析可选干净参考图，不要求分组区域有效，也不重新分组。
- 已完成：核对并记录 `CardBag018.groups.json` 的 32 个组号及原图左上角像素坐标种子；小缺口膨胀半径 4px，针对本图已核对的边缘封口使用 40px 内缩，仅影响分组识别，不改原图或 Piece 位置。分组记录绑定效果图 SHA-256；可选干净参考图同样绑定 SHA-256，换图后旧记录报错而非继续套用。手写数字不做不可靠 OCR；以后带手写数字的圈选图需先核对种子，普通无数字红圈可自动分组排序。
- 已完成：单包生成请求烘焙时只烘焙该包，不再触发全部卡包重烘焙。命令行 `GenerateCardBagFromCommandLine` 可加 `-bakeCardBagOutlines`，沿用现有烘焙算法与资源导入配置。批量菜单默认不烘焙的行为不变。
- 用户已确认并完成：`piece_087.png`（抱红色爱心睡觉的浅色猫）归 27 组，正式节点为 `Piece2702`；`piece_108.png`（伸懒腰的灰色猫）归 25 组，正式节点为 `Piece2509`。两片对应的区域面积分别为 `27/26=52.9%/47.1%`、`25/26=61.9%/38.1%`，已写入 `pieceOverrides` 并经生成结果核对，不再属于待确认项。
- 验证：Unity 编译通过，**12 项 EditMode 测试全部通过**（普通/显式命名、圈选与编号优先、指定组超过 14 片、跨线拒绝、种子重复区域、换图失效、手写编号待核对、未闭合轮廓、仅更新布局的隔离、对角线封口和有限补缝），结果在本机 `Logs/PreviewGroupingTests.xml`。生成前后按源 Sprite 逐片比较，192 片及 GameBoard/BoardTitle 的源 GUID、位置、尺寸、缩放、旋转、锚点、Pivot、父节点、Image 颜色、材质、组件类型均相同；Prefab GUID 和原 42 张描边 GUID 保留，新增 54 张描边及 Meta，合计 96 个 Sprite 全部加载成功。32 组编号连续，组内从左到右，无重复或漏片。真实生成与校验结果在 `Logs/CardBag018Generation.log`，逐片报告在 `Logs/CardBag018GeneratedPieces.json`，已目视检查按实际 Piece Alpha 上色的分组核对图 `Logs/CardBag018GroupingCheck.png`。未进入 Play Mode。
- 烘焙：仅处理 018，输出原 PNG 画布 `2600 x 3920`；最终边界 `assigned=14705 / unassigned=0 / ambiguous=1`。最后一项表示 1 个像素的两个候选评分接近，原烘焙器已为其选定唯一归属，不是缺失像素；没有为本次分组修改边界算法或导入规格。其他卡包 Prefab/描边、场景和 CardPacks 配置均未变化。
- 最终组片数（01~32）：`6/8/8/5/5/5/6/5/5/7/4/5/5/7/6/4/4/6/4/8/6/6/6/8/9/7/7/5/5/6/5/9`，正式编号范围 `Piece0101~Piece3209`。
- 收尾：移除一次性验证器后再次运行 12 项 EditMode 测试，全部通过（`2026-09-13 06:16:03~06:16:04 UTC`）。该批处理进程测试结束并保存报告后滞留在 Shutdown/许可断开阶段，确认命令行与 PID 属于本次测试后仅结束该进程；此前生成/烘焙批处理正常退出。源码和文档 `git diff --check` 通过；Prefab 差异只另外触发 Unity 原生空字段尾空格提示，不为此重写整个 YAML。
- 修改文件：生成器、圈选分组模块和对应 EditMode 测试、烘焙器的单包入口、018 Prefab 与该包描边、018 的 `.groups.json`/`.reference.png` 与 Meta、项目记录。用户替换的 Preview 原样保留，Unity 为其重新生成 Meta（新 GUID），该图不被运行时 Prefab 当 Sprite 使用。一次性真实资源验证器及其 Meta、分组核对图绘制脚本均已移除；核对报告留在本机忽略的 Logs 目录。
- 数据边界：不修改其他卡包、配置或玩家存档。最终分组变化后应重新开始该关验收，不能用旧 14 组编号的进行中存档验证；不自动迁移或删除存档。没有提交或推送。
- 下一步：在游戏中重新开始 CardBag018，按效果图编号依次核对 32 组及三套描边；重点检查第 25、27 组含已确认碎片。不要继续使用旧 14 组编号的进行中存档作为验收依据。分组规则和 018 生成均已完成，不要重复执行历史 005、Admin 或本轮生成任务；用户要求提交时再将源码、Prefab、生成图片与所有对应 Meta 一起提交并推送。

## 2026-09-12 CardBag005 保留四组并从右上开始

- 状态：005 组序、对应描边及生成器规则修改完成，Unity 验证通过，待重新开始该关进行游戏内验收。
- 用户确认：005 保留现有 19 片、4 个分组及每组成员，只调整组序为右上、左上、左下、右下；不按自动生成器重新分成 2 组。
- 修改：原组 `03 -> 01`、`04 -> 02`、`01 -> 03`、`02 -> 04`，对应右上、左上、左下、右下，新组片数依次 `4/4/4/7`。仅修改 Piece 名称中的组号并按正式编号排列 Hierarchy，组内编号、Sprite、位置、尺寸、缩放、旋转和材质不变；只重烘焙 005 的四组三套描边。
- 生成器：自动分组的蛇形顺序改为从右上开始，第一行从右向左、第二行从左向右，依次交替；组内从左到右、每组最多 14 片和显式 `PieceGGII` 人工命名规则保持不变，不重生成其他卡包。
- 数据边界：不修改或删除玩家存档。005 旧进行中记录使用旧 Piece 编号，重新开始该关后验收，不自动迁移。
- 修改文件：`Assets/Resources/CardBagPrefabs/CardBag005.prefab`、其 `Generated/PuzzleOutlines/CardBag005` 下 12 张 PNG、`Assets/Scripts/Editor/CardBagPrefabGeneratorEditor.cs`、任务记录与项目上下文；没有重生成其他卡包，也没有改卡包配置。工作期间出现的无关动态字体资源变更保留不动。
- 验证：Unity 中比较 19 片的全部组件序列化内容，改名前后一致；Git Prefab 差异只有 19 个组号和根子节点顺序。Prefab 与 12 张描边 GUID 不变，四组三套 Sprite 全部导入成功，最终轮廓归属 `assigned=4994 / unassigned=0 / ambiguous=0`。通过 `0/1/14/15/19/29/42/43/56` 片的自动分组测试，确认右上起步、逐行蛇形、每组不超过 14 片、组内从左到右和显式正式编号不被改写。一次性执行器及 Meta 已清理，未进入 Play Mode 或修改本地存档。
- 下一步：重新开始 CardBag005，依次确认右上、左上、左下、右下四组及对应描边；不要使用旧编号的进行中记录验收。现有完整覆盖生成仍遵循每组最多 14 片规则，本次保留四组是针对已有 Prefab 的局部组序调整。

## 2026-09-12 Admin 解锁全部卡包测试范围

- 状态：代码与 Unity 内存数据库验证完成，待 Admin 页面实际输入验收。
- 用户意图：Admin 的批量解锁用于测试，不再受 Demo 前 18 包限制；普通 Demo 默认范围保持不变。
- 修改：`10002003` 显示为“解锁所有卡包（测试）”，先保存全部卡包测试范围，再调用现有批量解锁逻辑；`10002001` 同样允许显示全部已配置卡包，单独执行不负责解锁；`10002002` 关闭测试范围并恢复前 18 包，隐藏而非删除后续卡包。
- 数据：当前存档的 SQLite `AdminSettings/Runtime` JSON 增加 `IgnoreBuildCardPackLimit`，默认 `false`。只有明确执行 Admin 显示全部或解锁全部命令才设为 `true`；保存失败同时恢复两个范围标记并中止批量解锁。不改变 SQLite 表结构，不清理或迁移玩家存档，不修改 Steam Demo App ID 或编译符号。
- 修改文件：`Assets/Scripts/Controller/AdminScene.cs`、`Assets/Scripts/Model/LocalDataStore.cs`、任务记录与项目上下文。
- 验证：Unity 编译通过；在独立内存 SQLite 中执行真实 Admin 命令，验证默认 Demo 上限 18、正式版默认无上限、全部 22 条配置解锁与显示、Admin 设置重新读取、切回 18 后保留全部记录、进行中/已完成状态与拼图数据不变、重复解锁幂等、保存失败回滚。无配置的 023 不生成记录。测试结束恢复所有临时静态状态，未写入实际玩家存档；一次性验证器已清理。
- 下一步：进入 Admin 输入 `10002003`，返回首页检查全部 22 包及系列折叠后的多页圆点；需要验证正常 Demo 时输入 `10002002`。本条取代此前“Admin 不能突破 Demo 上限”的限制。
...
- 用户意图：首次初始化默认英语，Demo 只显示前 18 包卡包。
- 修改：`GameDefine.DemoMaximumBagId` 从 `22` 改为 `18`；首页列表、预加载、完成数量、解锁及奖励候选沿用统一范围判定，001~018 全部获得后沿用现有任务终态，不再提供新任务。Admin 显示全部仍受构建上限限制；正式版编译分支不变。
- 语言：核对 `GameLocalization` 的初始化、重置和语言保存路径，首次无语言设置时已有默认 `en-US`，无需重复改写；不覆盖已保存语言，不跟随系统或 Steam 语言。
- 修改文件：`Assets/Scripts/Model/GameDefine.cs`、任务记录与项目上下文；不删除后续卡包资源、配置或玩家本地存档。
- 验证：当前 Standalone 仅定义 `STEAMWORKS_NET`，未定义正式版符号，使用 Demo 上限 18。核对首页数据、解锁、奖励候选及全收集判断均共用该范围；英语初始化无系统语言覆盖。通过当前 Unity 的 Assets/Refresh 完成脚本重编译和程序集重载，新增日志未出现编译错误；`git diff --check` 通过。未进入 Play Mode，也未改动未保存场景和 PlayerPrefs。额外尝试从 PowerShell 只读反射程序集常量受宿主 .NET Framework 与 Unity netstandard 2.1 类型解析限制，未作为验收依据。
- 下一步：重新进入首页核对卡包范围；使用未保存语言的全新用户环境确认初始英语，现有存档检查 019 之后不显示。
...
- 用户意图：所有卡包统一按原图尺寸制作，修复 CardBag018 包头原图 `2600 x 164` 却生成成 `2048 x 129` 的问题，并覆盖后续新增资源。
- 定位：生成器未提高 TextureImporter 的默认尺寸上限，包头又直接使用导入后的 `sprite.rect.size` 布局。扫描现有 23 张包头，只有 018 超过当前上限；018 的 `2600 x 3920` GameBoard 同样被降采样。
- 决策：仅针对 CardBag 源 PNG 和对应预览，按原图最长边提高导入上限，保留足够大的现有上限及压缩、材质等配置；包头新建时直接读取原图宽高。现有资源批量修正和局部更新统一将包头宽高匹配原图，保持下沿位置和已有手工分组、旋转缩放、投影、描边资源。
- 修改：生成器、源图导入回调和现有布局更新共用原尺寸规则；保留现有菜单，批量修正可通过 `MatchAllCardBagSourceSizes` 执行。018 的包头、GameBoard 和预览有效默认导入上限均提高为 `4096`；包头 Rect 从 `2048 x 129` 恢复为 `2600 x 164`，Y 从 `2024.5` 调整为 `2042`，下沿仍为 `1960`。其他包头无需更改。
- 修改文件：`Assets/Scripts/Editor/CardBagPrefabGeneratorEditor.cs`、`Assets/Resources/CardBagPrefabs/CardBag018.prefab`、`Assets/UI/CardBags/CardBag018/BoardTitle.png.meta`、`GameBoard.png.meta`、`Assets/UI/CardBags/Previews/CardBag018.png.meta`、任务记录和项目上下文。
- 验证：当前 Unity 完成脚本编译并运行临时验证器，23 个 Prefab 和 23 张包头全部通过原图尺寸检查；905 张卡包源图/预览的导入规则全部满足。比较修复前后的 Prefab YAML，包头 Rect 之外的内容全部一致，下沿位置不变；再次执行修复得到 `textures=0, titles=0`，文件内容保持一致。使用临时 `1200/2600/5000` 宽 PNG 验证新图自动导入、NPOT 禁止重采样、Windows 已启用覆盖的过小上限修正、不启用原本关闭的覆盖，以及保留已有 `8192` 上限。临时验证脚本、Meta 和测试资源已清理。此前工作区已有的 018 分组/描边删除及其他 Meta 改动均保留，不重建或恢复。
- 下一步：在 Unity 打开 CardBag018，确认包头宽度为 `2600` 并与棋盘上沿对齐；进入游戏核对清晰度。未来新增或替换卡包源图自动匹配导入上限，生成与局部更新时同步包头设计尺寸。
...
- 用户意图：托盘内容超宽并被用户向左滑动后，拿起 Piece 时若其左侧仍有 Piece 超出托盘可视左边界，则右侧 Piece 保持不动，由左侧 Piece 向右补空位；补位不能让最左 Piece 越过正常初始排位。拿起队尾 Piece 并成功让其离开托盘时，剩余整排还应在此前提下尽可能贴近托盘右边界并保留正常边距。
- 修改：拿起 Piece 时先检查其左侧队列的实际渲染边界；存在左侧越界且右侧仍有 Piece 时，右侧队列完全不动，直接按左侧最后一块和右侧第一块的实际渲染边缘计算左侧右移距离，使两侧重新保留标准间距，避免最左位置截断移动后在中间留下大空隙。拿取队尾时仍以最左 Piece 不越过初始左边界为上限；不存在左侧越界时，继续沿用右侧队列向左补位。若拿的是队尾且最终成功离开托盘，再根据剩余整排实际左右边界计算补位距离，优先补齐托盘右侧标准边距。所有移动复用现有 `0.5s` 缓动并同步更新 `StartPosition`。
- 边界：正确吸附、桌面组合吸附和合法桌面放置统一确认 Piece 离开托盘；错误回弹、主动拖回托盘和失焦取消仍恢复拿起前完整快照。左侧没有越出可视边界时保持原有右侧向左补位规则。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、任务记录和项目上下文。
- 验证：Runtime 与 Editor C# 工程均编译通过、`0` 错误；`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。当前设备没有可连接的 Unity 编辑器窗口，尚未完成实际托盘滑动验收。
- 下一步：使用 CardBag018/022 等超宽托盘，向左滑到左侧 Piece 越界后首次拿取中间 Piece，确认左侧最后一块与右侧第一块恢复标准间距且右侧位置不动；再拿取队尾 Piece，确认整排贴右时最左 Piece 不越过初始位置。另测试左侧没有越界时仍由右侧向左补位，以及错误回弹和主动回托盘仍完整恢复快照。

## 2026-09-09 拆包划线相交判定

- 状态：代码修改和静态验证完成，等待 MainScene Play Mode 交互验收。
- 用户意图：完整彩色卡包等待拆包时，滑动的起点和终点不再要求位于卡包内；只要起终点连线与卡包内容区上方约 `1/3` 的红框区域相交，抬起后即可执行开包动画，上下灰色封边不计入有效区域。
- 修改：保留卡包完整屏幕 Rect 用于原有最小滑动距离；另以顶部和底部各 `6%` 作为灰边内缩，在剩余内容高度中取上方 `1/3` 作为有效撕开区域。按下时允许从屏幕任意位置开始，移动出卡包不再取消；抬起时使用线段与轴对齐矩形的裁剪相交算法判定。区域内短点击开包的原交互继续保留，区域外点击无效。
- 修改文件：`Assets/Scripts/Controller/MainScene.cs`、任务记录和项目上下文。
- 验证：Runtime 与 Editor C# 工程均编译通过、`0` 错误；`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。当前设备没有可连接的 Unity 编辑器窗口，尚未完成实际滑动验收。
- 下一步：分别测试“左右两端都在卡包外但连线穿过红框”“斜线穿过红框”“线段只经过卡包下半部”“短点击红框内/外”；前三种中的前两种应开包，后两种滑线应无效，红框内短点击仍应开包。

## 2026-09-09 拆包滑光同步展开卡包高度

- 状态：场景配置修改和静态验证完成，等待 MainScene Play Mode 目视验收。
- 用户意图：卡包展开位置上移后，完整彩色卡包的撕开动画也要上移相同高度，使横向滑光重新对准实际撕口。
- 定位：`CardPackOpeningEffect` 日志显示卡包模型已按展开 Rect 自动拟合到 `1920 x 1080` 下的 `centerPixels.y=645`；未同步的是 MainScene `PackObject/fx_chai_w_001` 的独立场景光效。`PackCarousel` 在提交 `8b04e91` 中从 `Y=20` 改为 `Y=140`，实际差值为 `+120` 设计像素，而光效仍保留旧的 `Y=238.2`。
- 修改：只把 MainScene 中 `fx_chai_w_001` Prefab 实例根节点的本地 Y 从 `238.2` 改为 `358.2`，与展开卡包同步上移 `120`；不修改卡包模型拟合、Timeline、粒子内部 Transform、尺寸、材质、发射参数或动画节奏。
- 修改文件：`Assets/Scenes/MainScene.unity`、任务记录和项目上下文。
- 验证：确认场景仅有这一处 `fx_chai_w_001` 根位置覆写，差值与 `PackCarousel` 的历史上移量一致；`git diff --check` 通过。场景行为不涉及 C# 修改。
- 下一步：从完整彩色卡包展开页执行拆包，确认横向滑光中心与撕口重合，且星星、上下包体和 Piece 跳出位置保持原效果。

## 2026-09-09 系列叠放卡包改为单层原生投影

- 状态：代码修改和静态验证完成，等待 MainScene Play Mode 目视验收。
- 用户意图：首页系列槽位只显示一层自然投影，不能让前后两张卡包各自投影形成双层阴影，也不能用外接矩形拉伸封面轮廓模拟联合阴影。
- 根因：上一版 `SeriesPackShadow` 使用前后卡包旋转后的合并外接矩形绘制封面形状，拉伸后必然形成与卡包轮廓不符的大块矩形阴影。
- 修改：彻底移除运行时 `SeriesPackShadow` Image 和外接矩形计算。前层 `PackCover` 使用保留纹理、撕口和完成态参数的运行时材质副本，仅将投影 Alpha 设为 `0`；后层 `PackCover` 保持美术配置的原生材质与投影，因此系列槽位最终只有一层正常卡包阴影。普通单卡与 `PanelBagVol` 展开页不受影响。
- 生命周期：前层运行时材质由 `MainScene` 的 `PackageEntry` 持有，列表刷新和场景销毁时恢复原材质并释放，避免刷新后残留或泄漏。
- 修改文件：`Assets/Scripts/Controller/MainScene.cs`、`Assets/Scripts/View/PackCoverShadowEffect.cs`、任务记录和项目上下文。
- 验证：Runtime 与 Editor C# 工程均编译通过、`0` 错误；本机使用 .NET Framework 4.7.2 引用程序集验证，保留既有环境程序集警告。`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。当前设备没有可连接的 Unity 编辑器窗口，尚未完成实际投影目视验收。
- 下一步：首页显示至少一个已解锁 Vol2 的系列槽位，确认前层不再独立投影、后层只保留一份原生投影；同时检查呼吸动画、左右柔边裁切、选中隐藏和展开页单卡投影保持正常。

## 2026-09-09 托盘 Piece 回归动画层级保持

- 状态：代码修改和静态验证完成，等待 GameScene Play Mode 目视验收。
- 用户意图：托盘 Piece 拿起后若松手回到托盘，在回归动画完整结束前持续保持拿起时的高层级，不能突然落到其他托盘 Piece 的常驻光点下面。
- 根因：`EndDragging` 在判断回托盘之前就把拖拽 Piece 恢复到普通 `PieceSortingOrder`；单块从托盘返回时走托盘整体补位协程，该协程只移动位置，没有恢复拖拽层级，因此回程中会被其他 Piece 的 `sortingOrder + 1` 光点覆盖。
- 修改：回托盘判定改在恢复普通层级之前执行；托盘补位协程记录本次返回的被拖拽 Piece，保持其现有高层级到 `0.5s` 补位结束后再恢复。协程正常结束、无须移动或被中断时都会清理临时高层级，其他 Piece 补位、正确吸附、桌面放置和错误回弹流程不变。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、任务记录和项目上下文。
- 验证：Runtime 与 Editor C# 工程均编译通过、`0` 错误；本机使用 .NET Framework 4.7.2 引用程序集验证，保留既有环境程序集警告。`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。
- 下一步：从托盘拿起中间 Piece 后在托盘内、托盘边缘和托盘上方约半片高度处松手，确认整个返回过程都盖在其他 Piece 及其光点上方，到位后恢复正常同层显示；再测试横向滑动后的托盘回归。

## 2026-09-09 拍照输出斜边抗锯齿

- 状态：代码修改和静态验证完成，等待 MainScene/GameScene 实际拍照目视验收。
- 用户意图：消除导出照片中旋转棋盘顶部边缘明显的阶梯锯齿，同时保持现有构图、旋转、文件格式、尺寸、命名、保存和预览流程不变。
- 根因：拍照相机虽然启用了 `allowMSAA`，但目标 `RenderTexture` 使用默认单采样，并直接以最终 `1024 x 1024` 尺寸渲染旋转后的 UI 边缘，斜边没有足够采样覆盖。
- 修改：离屏合成先使用 `2048 x 2048`、`4x MSAA` 的临时 RenderTexture 渲染，再通过双线性采样缩回严格的 `1024 x 1024` 后读取并编码 PNG；两级临时 RenderTexture 均在成功、失败和异常路径统一释放。
- 修改文件：`Assets/Scripts/Model/CardPackPhoto.cs`、任务记录和项目上下文。
- 验证：Runtime 与 Editor C# 工程均编译通过、`0` 错误；本机缺少项目目标的 .NET Framework 4.7.1 Targeting Pack，使用已安装的 4.7.2 引用程序集验证，因此保留若干既有程序集解析和序列化字段警告。最终 Texture2D 和 PNG 仍固定为 `1024 x 1024`；`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。
- 下一步：分别在 MainScene 和 GameScene 对同一卡包拍照，放大查看旋转棋盘顶部及四角，确认锯齿明显改善且背景、图标、棋盘大小、颜色和预览没有变化。
...
- 用户意图：在“一键完成”左侧增加“自动拼图”；每次点击只将当前托盘中的一片 Piece 短暂飞入自己的正确凹槽，两个按钮共用显隐规则。
- 修改：运行时复用“一键完成”的按钮样式，在其左侧按按钮宽度加 `20px` 间距创建“自动拼图”；两者统一受 `AdminRuntimeSettingsUtility.ShouldShowTestCompleteButton` 控制，并在入场、拖拽、托盘滚动/补位、Piece 落位、切组和结算期间同步禁止操作，进入结算时同步隐藏。
- 自动放置：第一阶段新手引导优先选择指定 Piece，否则选择当前托盘顺序中的第一片有效 Piece；目标凹槽若被错误 Piece 占用，沿用现有顶回托盘规则。落位使用独立 `0.36s` 飞入时长，并复用正常落位的托盘补位/收起、存档、音效、绿色反馈、棋盘提交、切组和结算流程。
- 多语言：新增 `game.test_auto_puzzle` 的 18 种语言文本；两个调试按钮文字均保持单行并在 `14~28` 字号内自动适配。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、`Assets/Scripts/Model/GameLocalization.cs`、统一 spec、任务记录和项目上下文。
- 验证：Runtime 与 Editor 工程编译均通过，`0` 警告、`0` 错误；`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。
- 下一步：开启 Admin 的一键通关按钮设置后进入 GameScene，确认“自动拼图”位于其左侧且每次只飞入一片；连续测试托盘补位、最后一片收起、错误 Piece 被顶回、新手引导第一步、切组和最终结算，并切换长文本语言确认按钮不重叠。
...
- 用户意图：先放开现有后续卡包，便于继续操作和测试，不再让第 18 包成为当前构建的全收集终点。
- 修改：将默认 Editor/Demo 的最大可用 PackId 从 `18` 调整为 `22`。`CardBag019~022` 现已进入既有的常规解锁、奖励候选、首页列表、完成计数和任务终态判定；不会强制解锁或修改现有存档。`CardBag023` 虽有部分资源，但 `CardPacks.csv` 没有配置，因此继续不开放。`PUFFIES_STEAM_RELEASE` 正式版解除上限的逻辑不变。
- 修改文件：`Assets/Scripts/Model/GameDefine.cs`、任务记录、项目上下文和统一规格。
- 验证：确认 `CardPacks.csv` 当前有效配置为 `001~022` 共 22 包，019~022 的源资源、游戏 Prefab 和首页封面均存在；统一可用范围仍由 `GameDefine.IsCardPackAvailableInCurrentBuild` 控制。Runtime 与 Editor C# 工程编译通过，均为 `0` 警告、`0` 错误；`git diff --check` 通过。
- 下一步：使用接近原 018 上限的存档完成一局，确认可以按现有发包规则获得 019，首页列表继续显示任务；依次验证 019~022，获得 022 后才进入全收集终态。

## 2026-09-07 首页全收集文案恢复两行适配

- 状态：代码修改和静态验证完成，等待 18 语言 Play Mode 验收。
- 用户意图：首页全卡包终态的 `TaskContent2` 保留编辑器设置的两行显示，不能因运行时居中适配被强制压成单行。
- 修改：将 `TaskContent2` 纳入 `GameLocalization` 的任务描述分类，与 `TaskContent` 一样启用自动换行、最多显示两行，并以编辑器字号为上限；只有当前语言在设计字号下超过两行时才向下缩小。保留 Prefab 的 `500 x 100` 文本框、36 号设计字号、位置、居中方式、字体和材质。
- 修改文件：`Assets/Scripts/Model/GameLocalization.cs`、任务记录和项目上下文。
- 验证：静态确认 `TaskContent2` 不再进入默认单行适配，而是复用现有最多两行的任务文本算法；Runtime 与 Editor C# 工程编译通过，均为 `0` 警告、`0` 错误，`git diff --check` 通过。
- 下一步：在首页全收集终态切换英语、德语、俄语、法语和中文，确认短文保持设计字号，长文最多两行且不会超出文本框。
...
- 状态：代码修改和静态验证完成，等待 Play Mode 验收。
- 用户意图：结算页面判定当前构建内所有卡包均已解锁后，`TaskItem` 直接隐藏，不再显示任务内容。
- 修改：结算完成任务推进、待发任务奖励和首次完成自然奖励的发放判断后，再次检查当前构建卡包是否已全部解锁；若本轮刚好发出最后一个卡包，也会在奖励卡包动画开始前隐藏结算页 `TaskItem`。已有任务数据、首页全卡包终态和奖励发放逻辑不变。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、任务记录和项目上下文。
- 验证：`dotnet build Assembly-CSharp.csproj --no-restore` 与 `dotnet build Assembly-CSharp-Editor.csproj --no-restore` 均通过，`0` 警告、`0` 错误；`git diff --check` 通过。静态确认结算前已集齐时沿用现有任务隐藏逻辑，本轮奖励刚解锁最后一个卡包时会在奖励动画开始前追加隐藏 `TaskItem`。
- 下一步：使用仅剩最后一个未解锁卡包的 Demo 存档完成一局，确认最后卡包发出后结算页 `TaskItem` 隐藏；再用尚未集齐的存档确认任务仍正常显示。
...
- 修改：`CardPackDataUtility.AreAllCurrentBuildCardPacksUnlocked` 按当前构建配置核对每个可用 PackId 是否已经进入非 `Locked` 状态；达到条件后 `GameTaskUtility.TryGetCurrentTask` 对业务返回无活动任务，但保留现有 JSON 任务数据，正式版扩展卡包后可继续。MainScene 单独调用 `RefreshAllPacksCollected`，隐藏 `ProgressBg` 和整个 `BagBg`，将 `TaskContent` 拉伸到 TaskItem 内边距并居中显示“当前阶段的卡包已收集完毕！”，并提供 18 种语言文案。
...
- 用户意图：所有卡包尺寸的初始基础分统一扩大 `10` 倍，依赖分数的任务目标同步扩大 `10` 倍。
- 修改：`GameScoreUtility.GetBaseScore()` 七档分值由 `60/80/100/120/140/160/200` 调整为 `600/800/1000/1200/1400/1600/2000`；`TaskConfig.csv` 中唯一积分任务 `TaskType=1` 的目标循环由 `150|200|250|300` 调整为 `1500|2000|2500|3000`。贴纸数和完成卡包数任务不变。
- 计分影响：首次完成基础分、百分比加成实际分数和最终分均随基础分扩大；重玩仍只获得原始基础分的 `10%`，加成继续按原始基础分计算，百分比规则未改。
- 数据说明：任务实例会把生成时的目标值持久化；已存在的当前任务不会自动改写，新目标池从后续新生成的积分任务开始生效。按开发期规则验证全新任务时可手动删除 `persistentDataPath/LocalData.json`，SQLite 无需删除。
- 修改文件：`Assets/Scripts/Model/GameTaskUtility.cs`、`Assets/Resources/Configs/TaskConfig.csv`、任务记录、项目上下文、游戏设计需求和统一 spec。
- 验证：使用 `Import-Csv` 结构化读取确认三条任务分别保持 `1500|2000|2500|3000`、`45|60|80` 和 `2|3`，行数与字段未错位；`dotnet build Assembly-CSharp-Editor.csproj --no-restore` 通过，`0` 警告、`0` 错误；`git diff --check` 通过。
- 下一步：在 Play Mode 分别完成一次普通卡包和一次重玩，核对基础分、各项实际加分、最终分以及后续积分任务目标。
...
- 用户意图：结算页面的“总得分”标题与分数数字需要作为一个整体水平居中，而不是分别依赖场景中的固定坐标。
- 修改：`GameScene` 新增对 `RewardPanel/TaskBg2/TaskContent2` 总分标题的引用；保留编辑器现有标题与数字之间的 `5px` 间距，每次初始化或分数滚动刷新后，根据两个 TMP 文本当前内容的 `preferredWidth` 重新设置文本区域宽度与 X 坐标，使组合后的实际内容中心始终对齐 `TaskBg2` 中心。标题和数字原有 Y 坐标、字号、颜色、材质及滚分动画不变，不修改场景资源。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、任务记录。
- 验证：`Assembly-CSharp-Editor` 连带 Runtime 编译通过，`0` 警告、`0` 错误；`git diff --check` 通过。
- 下一步：在 GameScene Play Mode 触发结算，分别确认个位数、多位数以及不同语言下“总得分 + 数字”的组合中心均与结算面板中心一致。

## 2026-09-06 韩语愿望单按钮适配

- 状态：翻译调整和编译验证完成，等待 Play Mode 视觉验收。
- 用户意图：首页“添加愿望单”按钮的韩语文字不能超出异形按钮的有效文字区域。
- 修改：韩语从完整句式 `위시리스트에\n추가` 调整为按钮中常用的紧凑标签 `위시리스트\n추가`，保留“添加到愿望单”的含义和既有两行布局；不修改按钮 RectTransform、字号、材质或其他语言。
- 修改文件：`Assets/Scripts/Model/GameLocalization.cs`、任务记录。
- 验证：第一行由 6 个韩文音节缩短为 5 个，适配 `BtnWishList/TextTitle` 的 `250 x 100` 文本区域；`Assembly-CSharp-Editor` 连带 Runtime 编译通过，`0` 警告、`0` 错误。
- 下一步：在 MainScene Play Mode 切换韩语，确认两行文字均完整位于蓝色按钮图形内。
...
- 修改：新增 `Assets/Audios/SFX_CameraShutter.mp3`，并按反馈替换为老式机械相机的两段“咔、嚓”声：第一下偏厚，短暂停顿后第二下更清脆；音频为 `48 kHz` 双声道、`192 kbps`、约 `0.23s`。新增并固定 `.meta` GUID，将 AudioClip 按现有排序加入 `AudioCatalog.asset`。MainScene 与 GameScene 共用的 `CardPackPhoto.PlayPhotoFlash` 在白色闪光完成首帧准备、开始淡入的同一帧播放快门音效；点击按钮、保存失败或预览出现时不会重复播放。
...
- 验证：最终音频峰值约 `-7.0 dB`、平均约 `-28.2 dB`，开头立即存在第一段机械点击，没有前导静音；两段有效机械声约为 `0~41ms` 与 `112~162ms`。GUID 在项目中唯一且 AudioCatalog 引用一次；`Assembly-CSharp-Editor` 连带 Runtime 编译通过，`0` 警告、`0` 错误。
...
- 用户意图：结算页面分数个位数的增长速度提高 `40%`，并让 `SFX_ScoreIncrease.mp3` 只在数字实际上涨期间发声；阶段停顿时暂停，下一段上涨时恢复。
- 修改：结算分数每一段滚动统一应用 `1.4x` 速度，计算方式为原滚动时长除以 `1.4`。基础分有效滚动时长由 `1.20s` 变为约 `0.86s`，每条加成由 `1.08s` 变为约 `0.77s`；最终补差分数同样提速。滚分音效改用可暂停循环：每段开始时播放或从原位置续播，分数停止变化时立即暂停，全部计分结束或 GameScene 销毁时彻底停止。
- 保留：最终分数、各加成分数、计分公式、加成文案提前停顿、最终分停留以及不随分数滚动的独立任务进度动画均不变。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、`Assets/Scripts/Model/AudioManager.cs`、任务记录、项目上下文和统一 spec。
- 验证：基础分、所有加成和最终补差均只调用统一 `AnimateSettlementScoreRange`，后续阶段同样应用 `1.4x` 倍率；`SFX_ScoreIncrease.mp3` 已无 `PlayOneShot` 调用；`Assembly-CSharp-Editor` 连带 Runtime 编译通过，`0` 警告、`0` 错误。
- 下一步：完成一次包含基础分和至少两项加成的结算，确认每次数字停顿时声音同步暂停、下一段上涨时无缝恢复、最终计分后不再残留。

## 2026-09-05 拼图发牌音效整段循环

- 状态：代码修改和编译验证完成，等待 Play Mode 试听。
- 用户意图：`SFX_PieceDeal.mp3` 每轮发牌只启动一次，并在拼图持续发牌期间循环播放，发完牌立即停止。
- 修改：首次进入关卡和后续组切换两条发牌流程都在 Piece 开始飞向托盘时启动循环音效，在整组飞行动画结束时停止；删除原来的逐片 `PlaySfx` 调用。发牌协程被中止或 GameScene 销毁时也会清理该音效；停止操作只匹配 `SFX_PieceDeal.mp3`，不会误停其他循环音效。
- 保留：碎片数量、顺序、起点、终点、缩放、飞行时间和错峰间隔均未修改；没有有效 Piece Renderer 时不播放。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、`Assets/Scripts/Model/AudioManager.cs`、任务记录、项目上下文和统一 spec。
- 验证：全工程已无 `SFX_PieceDeal.mp3` 的逐片 `PlaySfx` 调用；首次发牌和组切换各只有一组循环开始/停止边界；`Assembly-CSharp` 编译通过，`0` 警告、`0` 错误；`git diff --check` 通过。
- 下一步：在 Play Mode 分别试听首次进入关卡和完成一组后的下一组发牌，确认循环从首片起飞开始、最后一片落位立即停止。
...
- 状态：编辑器静态 UI 结构、语言项 Prefab 和 18 项内容已完成；本轮未接入语言切换与文本翻译逻辑。
- 用户意图：游戏正式只支持 18 种语言，`PanelLanguage` 一页即可完整展示；沿用 `LanName1` 选中样式和 `LanName2` 常规样式，取消滚动列表，保留可复用 `LanNameItem.prefab`。
- 编辑器结构：`PanelLanguage/LanguageList/Content` 使用静态两列九行布局。已删除语言区域的 `ScrollRect`、`Viewport`、`RectMask2D` 和透明承载 Image；`LanguageList` 保持用户调整后的位置 `(0, 90)` 和尺寸 `492 x 850`。Content 使用 `GridLayoutGroup + ContentSizeFitter`，单元尺寸 `234 x 70`、横向间距 `24`、纵向间距 `18`，计算高度 `782`，在 LanguageList 内上下各留 `34px`，无需滚动即可完整显示。
...
- 列表内容：Content 中只保留 18 个 Prefab 实例，按用户确认顺序逐行排列：`zh-CN`、`en-US`、`ru-RU`、`es-ES`、`es-419`、`pt-BR`、`pt-PT`、`de-DE`、`ko-KR`、`fr-FR`、`ja-JP`、`tr-TR`、`zh-TW`、`pl-PL`、`it-IT`、`uk-UA`、`vi-VN`、`th-TH`。名称继续使用 `LanNameItem_<language-code>`，显示各语言原生名称；默认 `en-US` 使用 `LanName1`，其余使用 `LanName2`。
...
- fallback 材质统一：`NotoSans LGC/SC/JP/TC/KR/Thai SDF` 原资源按 `90px` 采样，而现有 `NotoSansSC-Regular SDF` 为 `36px`，导致相同描边参数随 fallback 字形缩小后明显变细；单纯放大材质还会受原 Atlas `5px` SDF 边距裁切并产生矩形色块。最终将 6 套 fallback 保留原字符集合、源 TTF、GUID 和 Atlas 尺寸，统一按 `36px + 5px padding + SDFAA_HINTED` 重新烘焙，并关闭 TMP 的 `Match Material Preset`，让 fallback 使用各自正确的 Atlas 材质。6 套材质统一使用 `TMP SDF Mobile`、`OUTLINE_ON + UNDERLAY_ON`、`OutlineWidth 0.45` 和 `FaceDilate 0.2`；因 `NotoSansKR-Regular` 的韩文字面相对同字号中文和拉丁文先天偏细，KR 材质单独将 `FaceDilate` 微调为 `0.35`，只增厚字面，不改文字组件和字号。
- 层级：LanguageList、Content、Prefab 根和文字节点全部使用 `UI` Layer。原先直接挂在 `PanelLanguage` 下的两个模板对象已由固定列表替代。
- 验证：场景内恰好 18 个语言项，Content 子节点顺序与用户清单完全一致；旧 11 个实例、ScrollRect、Viewport、RectMask2D 对应对象和 fileID 引用均已清除，LanguageList 只保留 RectTransform。Grid 固定两列九行，高度计算与序列化尺寸均为 `782`。Prefab 两套 TMP 保持原字体 GUID `07169799dbf3be7428d02d725795d190` 和共享材质 fileID `381051975239993251`；Runtime/Editor 编译均为 `0` 警告、`0` 错误。待 Unity 刷新后做最终画面确认。
- 下一步：接入本地化数据和语言选择控制时，给 18 个根 Button 绑定语言代码，点击后只切换当前项/前一项的 `LanName1/2`，持久化当前语言并刷新所有本地化文本。
...
- 状态：代码修改和编译验证完成，等待 Play Mode 试听。
- 用户意图：整关最后一块拼图放下并确认正确时，立即开始播放 `SFX_PuzzleComplete.mp3`，不等待吸附动画、绿色效果或结算流程。
- 修改：正确落子分支在本次全部成员标记为 `IsPlaced` 后判断“当前组已完成且为最后一组”，成立时立即播放完成音效；`TryAdvanceGroup` 仍在吸附和绿色效果结束后显示结算，但不再重复播放完成音效。
- 保留：普通正确落子音效、吸附动画、绿色滑光、组切换、存档和结算时序均未修改。
- 修改文件：`Assets/Scripts/Controller/GameScene.cs`、统一 spec、任务记录与项目上下文。
- 验证：全工程 `SFX_PuzzleComplete.mp3` 只剩正确落子分支中的一个调用；Runtime/Editor 编译通过，`0` 警告、`0` 错误；`git diff --check` 通过。
- 下一步：在 Play Mode 放下整关最后一块，确认鼠标松手判定正确后立即播放且全程只播放一次。

## 2026-09-04 碎片分发音效按片播放

- 状态：历史方案，已由 2026-09-05“拼图发牌音效整段循环”取代。
