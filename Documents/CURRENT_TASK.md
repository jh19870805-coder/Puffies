...
- 新增通用 `HintDashedOutlineGraphic` UI Mesh 组件，从目标 Piece Sprite 的实际 Alpha 像素获取有序闭合轮廓，并通过时间相位生成持续滚动的绿色虚线。
- 虚线对象运行时复制对应 `GrooveRect` 的布局并放到同级最上方，不修改 CardBag Prefab，也不需要新增或重新烘焙提示蒙版。
- 原 Shader 按角度或局部切线分段都会让不规则轮廓上的实际线段长度不一致；现改为沿轮廓累计真实距离生成固定长度实线段，并将闭合轮廓余量均摊到间隔中，避免首尾出现半截线。
- 根据最新视觉反馈将虚线实线长度调整为 `20` 像素，基础间隔调整为 `15` 像素。
- 线段周期增大后原 `8` 像素/秒滚动速度视觉上接近静止；根据最新反馈将滚动速度提高到 `60` 像素/秒。
- Sprite Physics Shape 会被 Unity 自动简化，导致提示描边出现偏差和漏边；现改为通过 GPU 读取不可读贴图的实际 Alpha，按 `10%` Alpha 阈值提取像素边界，并以不超过 `0.75` 原图像素的误差简化路径。路径在当前 GameScene 内按 Sprite 缓存，离开场景时清空；Physics Shape 仅作为读取失败时的回退。
- `BtnTips` 使用切换逻辑：第一次点击显示提示，提示存在时再次点击立即取消；取消提示不会恢复本局未使用提示的积分资格。
- 只有找到有效未完成碎片并显示提示后，才将 `_wasHintUsed` 设为 `true`，取消本局未使用提示的 5% 加成。

## 修改文件

- `Assets/Scripts/Controller/GameScene.cs`
- 删除 `Assets/Resources/Effects/HintDashedOutline.shader` 及其 Meta；提示线不再使用 Shader。
...
- 提示轮廓由通用 UI Mesh 组件读取 Sprite 实际 Alpha；不修改贴图 Read/Write 设置，也不为每个卡包增加美术资源。
...
- 旧提示 Shader GUID 在场景、Prefab、Material 和 Asset 中均无序列化引用，删除后不会产生 Missing Shader 引用。
- Alpha 轮廓读取、闭合路径追踪、路径简化和 Physics Shape 回退代码通过 C# 编译；实际边界贴合度仍需 Play Mode 视觉复测。
- 当前后台 Unity 进程未提供可见窗口，尚未完成等长虚线实际显示、虚线速度、线宽和抖动手感的 Play Mode 验收。
- 不涉及持久化结构变化，无需删除 `LocalData.db` 或 `LocalData.json`。

## 下一步

1. 等 Unity 完成脚本导入后重新进入 GameScene Play Mode。
