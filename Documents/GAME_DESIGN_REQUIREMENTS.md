...
- 点击 `PanelBagSelect/BtnPlay` 或重玩后，首页内容退场并切换到与 GameScene 同源的 `BgGame` 开包舞台；卡包定场后等待玩家沿顶部封口向右横划，不得自动开包。
- 有效横划后同时播放六层开包 Animator 和 `CardPackDismantle_001` 拆包粒子，动画结束后进入 GameScene。
...
| 日期 | 变更 |
|---|---|
| 2026-07-24 | 按参考视频确认首页到游戏流程：选择页柔化背景，Play 后进入独立开包舞台，玩家沿封口向右横划触发拆包，随后播放 GameScene 棋盘、托盘和 Piece 入场。工程保持 `2560 x 1440` 横屏布局。 |
