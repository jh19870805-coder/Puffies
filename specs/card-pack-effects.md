...
3. 当卡包在开包舞台定场后，除放大的选中卡包外，其他列表卡包和尺寸图标必须全部隐藏；系统循环显示一个沿卡包顶部封口从左向右移动的无文字手势提示，并等待玩家操作，不得自动开包。
4. 玩家轻点放大的卡包，或从卡包顶部封口左侧开始并完成有效的向右横划时，系统必须隐藏提示并只触发一次开包；横划距离不足或明显偏离封口的操作不得按横划触发，松手后提示恢复。
...
9. GameScene 场景加载或首帧初始化发生卡顿时，入场动画不得按真实卡顿时长跳过；棋盘滑入和当前组 Piece 飘落必须至少从起始姿态渲染后再开始推进。
...
- GameScene 将全部入场对象设置到起始姿态后先保留两个渲染帧，入场时钟的单帧推进量限制为 `1/30s`，隔离场景加载和首帧资源初始化尖峰。
