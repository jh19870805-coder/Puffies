...
- 新增或修改 CardBag Prefab 或 Piece 贴图后，执行 **Puffies -> Bake CardBag Outlines**。
- 执行 **Puffies -> Generate CardBag Prefabs** 打开批量窗口；窗口会发现所有符合命名规则且资源完整的 `CardBagNNN` 目录。
...
- 批量 Prefab 生成完成后，先手工完成顺序节点分组，再执行 **Puffies -> Bake CardBag Outlines**。
