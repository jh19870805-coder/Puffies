...
## 2026-08-25 - 选中卡包背景无染色半透明虚化

### 需求

1. WHEN 卡包进入居中选中页面 THEN 背景 SHALL 保留首页原始色彩，只呈现柔和虚化，不得形成明显灰层或白雾。
2. 虚化层 SHALL 以半透明方式与原画面混合，不得叠加黑色或白色染色层；选中卡包、按钮、分页内容和其他交互 SHALL 保持现有层级与行为。
3. WHEN 从选择页进入开包舞台 THEN 虚化层 SHALL 从当前透明度连续淡出，不得瞬间恢复全不透明。

### 实现与验证

- [x] 1. 确认 `PanelBagSelect` 根 Image 引用黑色 `ImgMaskBlack.png`；`0.34/0.12/0.06` 均会产生不同程度灰感，因此最终将场景与运行时 Alpha 设为 `0`，只保留 Raycast。
- [x] 2. 将运行时虚化截图设为纯白不染色和 `0.45` Alpha，让原首页透出 `55%`；相比无黑层时的 `0.6` 降低亮色扩散形成的白雾，转场淡出同步以 `0.45` 为起始 Alpha。
- [x] 3. 不修改虚化降采样层级、前景排序、卡包动画和选择页交互，并完成 Runtime/Editor 编译与 Git 差异检查。
- [ ] 4. 在 MainScene Play Mode 与参考图对比背景色彩、虚化程度及进入开包舞台时的淡出衔接。
