...
11. `PackSize` SHALL 在 Prefab 中按展开态比例制作；首页列表先按卡包比例转换布局，再使用 `0.75` 列表视觉系数等比缩小标签尺寸。缩小后的标签左边缘 SHALL 与列表卡包左边缘对齐，纵向布局 SHALL 不因额外视觉缩放而漂移，不得产生非等比拉伸。
...
- [x] 8. 将列表 `PackSize` 缩放改为统一比例，并根据缩放后宽度与 Pivot 显式左对齐列表封面。
