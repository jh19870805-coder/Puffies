...
1. WHEN 彩色撕开卡包进入游戏并开始下移退场 THEN 卡包退场速度 SHALL 提高一倍，完整彩色卡包与灰色撕开的退场参数不得改变。
2. WHEN 彩色撕开卡包开始下移 THEN 卡包内随跨场景 Canvas 带入的展示碎片 SHALL 相对卡包向下收进撕口；只有低于撕口后才隐藏，不得在下移首帧直接消失。
3. WHEN 彩色撕开卡包完成首页背景交接并准备进入游戏 THEN 原最短静止等待 SHALL 减少 `0.3s`；当前 `0.255s` 等待归零，但 GameScene 预加载安全等待保持不变。
4. WHEN 红框内已准备好的真实 Piece 从卡包位置收向托盘 THEN 彩色撕开单片飞行时长 SHALL 在当前 `0.69s` 基础上减少 `0.3s` 至 `0.39s`。
5. WHEN 彩色撕开进入 GameScene THEN 全部真实 Piece SHALL 同时创建在卡包中心附近的小范围散点，并立即使用最终 `TrayScale` 与正常 Alpha；GameScene 完成两帧稳定及托盘目标缓存后才允许跨场景卡包开始下移，使真实 Piece 始终先存在于卡包后方并随卡包移开自然露出，不得在卡包越过碎片位置后补显。WHEN 卡包下移约自身显示高度 `72%` THEN 真实 Piece SHALL 开始按 `0.027s` 错峰飞向各自托盘终点。
6. GameScene 棋盘、托盘、完整彩色卡包流程和灰色撕开流程 SHALL 保持不变。
7. WHEN 彩色撕开卡包经过真实 Piece 起飞点 THEN 卡包 SHALL 保持相同下移速度并连续移出屏幕，不得在两段交界处减速、停顿或跳变。

### 设计与任务

- [x] 1. 为彩色撕开分支独立计算退场时长和静止等待，不修改共用基础参数。
- [x] 2. 跨场景转场仅在彩色撕开分支让展示碎片相对卡包向下收进撕口，并在下收完成后隐藏。
- [x] 3. 为彩色撕开向 GameScene 传递独立的真实 Piece 收拢时长，保持其他状态与错峰参数不变。
- [x] 4. 彩色撕开的真实 Piece 使用卡包中心附近的小范围散点起点，在卡包移动前以正常 Alpha 完成准备，错峰改为 `0.027s`；展示碎片不参与飞向托盘。
- [x] 5. 彩色撕开在约 `72%` 卡包高度处提前发牌，并以相同线速度连续完成剩余退场；最终距离按实际 Canvas 底边计算，保证整个卡包离屏。
- [ ] 6. Runtime/Editor 编译与差异检查已通过；仍需在 Play Mode 对比三种卡包状态。

## 2026-08-27 - 完整彩色卡包碎片蹦出节奏

### 需求

1. WHEN 完整彩色卡包拆包时第一组临时碎片到达各自启动时间 THEN 碎片 SHALL 直接显示，不得使用 Alpha 渐显。
2. WHEN 临时碎片开始蹦出 THEN SHALL 从最终尺寸的 `40%` 缓动放大到 `100%`。
3. 临时碎片 SHALL 只从撕口起点向上移动少量距离，不得继续使用原卡包高度 `24%~27.6%` 的高位蹦出。
4. 临时碎片 SHALL 从卡包模型后方露出；全部碎片使用同一个水平中心、竖向终点和旋转，不得在拆包阶段横向散开。
5. WHEN 临时碎片完成蹦出 THEN SHALL 保持重叠停留在共同起点；进入 GameScene 后由真实 Piece 接管，并继续使用完整彩色卡包独立保留的 `0.0135s` 错峰和 `0.39s` 单片时长飞向托盘。
6. 彩色撕开、灰色撕开和临时碎片逐片启动节奏 SHALL 保持不变。

### 设计与任务

- [x] 1. 临时碎片初始禁用 Renderer，到各自启动时间直接启用并保持 Alpha `1`。
- [x] 2. 临时碎片 Scale 从 `0.4` 缓动到 `1`，复用现有位移进度。
- [ ] 3. 竖向位移已降至卡包高度约 `8%~9.2%`，Runtime/Editor 编译与差异检查通过；仍需 Play Mode 验收。
- [x] 4. 临时碎片改为模型后方的共同位置与统一旋转，取消横向散开和高度变化。
- [ ] 5. 完整彩包 GameScene 接管后已复用彩色撕开的 Piece 飞行时长与错峰，Runtime/Editor 编译与差异检查通过；仍需 Play Mode 验收。
