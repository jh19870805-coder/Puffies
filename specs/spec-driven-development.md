...
1. WHEN CardBag001 新手引导进入第三步 THEN 提示框整体 SHALL 在现有动态定位结果基础上向左移动 `48` 个、向下移动 `20` 个设计像素，避免提示背板遮挡 `BtnTips`。
2. WHEN 提示框移动 THEN 箭头 SHALL 继续独立指向 `BtnTips` 左边缘外的既定位置，并在按钮垂直中心基础上向下移动 `20` 个设计像素；按钮、箭头动画、提示框尺寸、文案和其他引导步骤 SHALL NOT 改变。
3. IF 左移后的提示框接近屏幕边缘 THEN 系统 SHALL 继续使用现有安全边距限制，保证提示框完整位于画面内。

### 设计与任务

- [x] 只在 `TutorialStage.HintIntroduction` 动态定位分支增加 `48` 设计像素左移和 `20` 设计像素下移。
- [x] 保留箭头按按钮实际矩形独立重定位以及 `16` 设计像素按钮间距，并将箭头目标向下调整 `20` 设计像素。
- [x] 编译 Runtime/Editor 程序集并完成静态差异检查。
- [ ] 在 Unity Play Mode 验证第三步提示背板不再遮挡按钮，箭头仍准确指向按钮。
...
1. WHEN 生成任务类型 1 THEN 系统 SHALL 使用“完成任意拼图包，收集 N 分”，并从 `1500|2000|2500|3000` 按顺序取值，取完后循环回 `1500`。
...
- `TaskConfig.csv` 收敛为三个启用模板：类型 1 使用任意尺寸和目标池 `1500|2000|2500|3000`；类型 2 使用任意尺寸和目标池 `45|60|80`；类型 3 使用指定尺寸池 `2|3`（`S|M`）和目标池 `2|3`。
...
- [x] 3. 修改 `CardPackRewardFlyTransition`，整实例接管 `BagRewardItem`，落位完整播放实例内粒子后同帧替换为真实卡包。
...
10. WHEN GameScene 的 Windows 客户区尺寸变化 THEN 系统 SHALL 等待最新尺寸至少稳定两帧后，再使用稳定后的 Canvas 边界完整重算相机、棋盘、托盘和托盘 Piece；若新手引导正在显示，最终布局完成后 SHALL 按新坐标重建提示框、箭头与焦点层。
...
- [x] 11. 修复 GameScene 在尺寸变化当帧读取旧 Canvas 边界的问题：连续变化只保留最新尺寸，稳定两帧并处于无拖拽/互斥动画的安全时机后再次配置视口、强制刷新布局并重算棋盘与托盘；同步重建活动中的新手引导层。
- [ ] 12. 在 Windows Player 游戏过程中验证窗口化、最大化、恢复及连续横向/纵向拉伸，确认当前棋盘、托盘和教程目标始终完整可操作。
...
1. WHEN GameScene 结算入场、积分滚动或奖励卡包动画尚未完成 THEN 系统 SHALL 忽略空白区域、完成按钮和拍照按钮产生的交互，不得隐藏、移动或提前交接奖励卡包。
2. WHEN 结算动画全部完成 THEN 系统 SHALL 恢复 `BtnFinish` 与 `BtnCamera` 的正常交互，既有结算动画节奏和视觉参数保持不变。
3. IF 任一并行奖励子动画未正常返回完成信号 THEN 结算主流程 SHALL 在有限时间内停止相关子动画、恢复奖励区最终可见状态并继续显示操作按钮，不得永久等待。

### 设计与任务

- [x] 1. 在 `RewardPanel` 根节点统一管理结算交互锁，动画期间保留全屏射线承接但禁用全部子级交互。
...
- `SFX_CameraShutter.mp3` 为 `48 kHz` 双声道、`192 kbps`、约 `0.23s`，峰值约 `-7.0 dB`、平均约 `-28.2 dB`，开头直接存在第一段机械点击；两段有效声分别约为 `0~41ms` 和 `112~162ms`。
