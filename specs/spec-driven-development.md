...
1. WHEN 彩色撕开或灰色撕开卡包进入游戏并开始下移退场 THEN 两者 SHALL 使用相同的 `0.345s` 线性连续退场参数；完整彩色卡包的独立拆包流程不得改变。
...
5. WHEN 彩色撕开或灰色撕开进入 GameScene THEN 全部真实 Piece SHALL 同时创建在卡包中心附近的小范围散点，并立即使用最终 `TrayScale` 与正常 Alpha；GameScene 完成两帧稳定及托盘目标缓存后才允许跨场景卡包开始下移，使真实 Piece 始终先存在于卡包后方并随卡包移开自然露出，不得在卡包越过碎片位置后补显。WHEN 卡包下移约自身显示高度 `72%` THEN 真实 Piece SHALL 开始按 `0.027s` 错峰飞向各自托盘终点。
6. GameScene 棋盘、托盘和完整彩色卡包流程 SHALL 保持不变；灰色撕开只统一动画节奏，不得新增假碎片。
7. WHEN 彩色撕开或灰色撕开卡包经过真实 Piece 起飞点 THEN 卡包 SHALL 保持相同下移速度并连续移出屏幕，不得在两段交界处减速、停顿或跳变；两种状态 SHALL 共用无额外停顿、`0.345s` 总下落时长、约 `72%` 卡包高度起飞点、`0.027s` Piece 错峰和 `0.39s` 单片飞行时长。ONLY IF 卡包为彩色撕开 THEN 系统 SHALL 下收并隐藏首页展示用 `ProgressPieces`；灰色撕开不得执行假碎片显隐操作。

### 设计与任务

- [x] 1. 彩色撕开和灰色撕开共用退场时长、静止等待、线性下落、起飞点和真实 Piece 飞行参数。
...
- [ ] 5. Runtime/Editor 编译与差异检查已通过；仍需在 Play Mode 对比完整彩包、彩色撕开和灰色撕开。
...
6. 完整彩包临时碎片逐片启动节奏 SHALL 保持不变；彩色撕开与灰色撕开的跨场景退场节奏按上一节统一。
