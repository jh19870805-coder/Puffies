...
5. WHEN 彩色撕开进入 GameScene THEN 全部真实 Piece SHALL 同时创建在卡包中心附近的小范围散点；卡包下移约自身显示高度 `72%` 时，真实 Piece SHALL 提前开始按 `0.027s` 错峰飞向各自托盘终点。
...
- [x] 4. 彩色撕开的真实 Piece 使用卡包中心附近的小范围散点起点，错峰改为 `0.027s`；展示碎片不参与飞向托盘。
