...
2. WHEN 彩色撕开卡包开始下移 THEN 卡包内随跨场景 Canvas 带入的展示碎片 SHALL 在同一帧立即隐藏，不得与 GameScene 创建的真实 Piece 同时显示或随卡包继续下移。
...
5. WHEN 彩色撕开进入 GameScene THEN 全部真实 Piece SHALL 同时创建并重叠停留在卡包中心共同起点；卡包开始下落时展示碎片立即隐藏，真实 Piece 随后从共同起点按 `0.0135s` 错峰飞向各自托盘终点。
6. GameScene 棋盘、托盘、完整彩色卡包流程和灰色撕开流程 SHALL 保持不变。

### 设计与任务

- [x] 1. 为彩色撕开分支独立计算退场时长和静止等待，不修改共用基础参数。
- [x] 2. 跨场景转场仅在彩色撕开分支于卡包下移首帧立即隐藏卡包内展示碎片。
- [x] 3. 为彩色撕开向 GameScene 传递独立的真实 Piece 收拢时长，保持其他状态与错峰参数不变。
- [x] 4. 彩色撕开的真实 Piece 使用共同中心起点并恢复 `0.0135s` 错峰；展示碎片不参与飞向托盘。
- [ ] 5. Runtime/Editor 编译与差异检查已通过；仍需在 Play Mode 对比三种卡包状态。
